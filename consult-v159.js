...
(function(){
  'use strict';
  const base=typeof window.renderConsultingWorkspace==='function'?window.renderConsultingWorkspace:null;
  ...
  function enhance(){
    ...
    const bridge=window.AiderConsultBridge,data=bridge?.snapshot?.();
    const selected=(data?.consultingClients||[]).find(row=>row.id===bridge?.selectedId?.());
    if(!selected)return;
  ...
  if(base)window.renderConsultingWorkspace=function(){base();enhance()};
  document.addEventListener('click',async event=>{
    const stageButton=event.target.closest('[data-consult-v159-stage]');
    if(stageButton){
      try{const saved=await window.AiderConsultBridge?.setStage?.(stageButton.dataset.clientId,stageButton.dataset.consultV159Stage);if(saved)window.AiderConsultBridge?.notify?.('컨설팅 단계를 저장했습니다.')}catch(error){window.AiderConsultBridge?.notify?.(window.AiderConsultBridge?.errorMessage?.(error)||String(error?.message||error))}
      return;
    }
    ...
    if(pdf){
      const data=window.AiderConsultBridge?.snapshot?.(),client=(data?.consultingClients||[]).find(row=>row.id===pdf.dataset.consultV159Pdf);if(!client)return;
      ...
      const popup=window.open('','_blank');if(!popup){window.AiderConsultBridge?.notify?.('팝업 차단을 해제한 뒤 다시 시도하세요.');return}try{popup.opener=null}catch{}
  ...
  if(base)window.renderConsultingWorkspace();else enhance();
})();
