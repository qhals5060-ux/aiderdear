...
const packaging = read('../../tools/package-pc-v178.ps1');
...
test('v178 release: site, PC and Android metadata/download advance together', () => {
  assert(index.includes('<meta name="aiderlog-build" content="v178">'));
  assert(index.includes('<meta name="aiderlog-android-build" content="v178">'));
  assert(index.includes('href="./AiderLog-v178.apk" download="AiderLog-v178.apk"'));
  assert(!/AiderLog-v(?!178\b)\d+\.apk/.test(index));
  for (const edition of ['Modern','Editorial']) assert(index.includes(`href="./AiderLog-${edition}-v178-site-files.zip" download="AiderLog-${edition}-v178-site-files.zip"`));
  assert(!/AiderLog-(?:Modern|Editorial)-v(?!178\b)\d+-site-files\.zip/.test(index));
});

test('v178 release: all local site entry dependencies exist and use current release queries', () => {
  for(const value of localDependencies(index)){
    ...
    if(url.searchParams.has('v'))assert.equal(url.searchParams.get('v'),'178',value);
  }
});

test('v178 release: ESTATE and co-broker retain source filenames while entry queries advance', () => {
  for(const name of ['estate-v171.js','estate-v171.css','estate-calendar-v171.js','estate-directory-v171.css','estate-workflow-v171.css','estate-cobroker-v173.css']){
    assert(index.includes(`./${name}?v=178`));assert(shell.includes(`./${name}?v=178`));
  }
  assert(!/estate(?:-(?:client|domain|directory|workflow|calendar|public))?-v175/.test(index+sw+packaging));
});

test('v178 release: retained and new browser modules are packaged and cached with and without queries', () => {
  for(const name of [...retained,...added]){
    assert(fs.existsSync(path.join(root,name)),name);assert(shell.includes(`./${name}`),name);
    assert(shell.includes(`./${name}?v=178`),name+' query');assert(packaging.includes(`'${name}'`),name+' packaging');
  }
...
test('v178 release: new shared UI modules are mounted once, model modules are statically imported', () => {
...
test('v178 release: service worker has new version key, unique existing pre-cache paths', () => {
  assert.equal(vm.runInContext('CACHE',context),'aiderlog-v178-site-media-widgets');
...
test('v178 release: private records, share links, API responses and binaries are never pre-cached', () => {
...
test('v178 release: service worker bypasses API, authenticated, cross-origin, binary and non-GET requests', () => {
  for(const options of [
    {url:'https://aiderdear1.vercel.app/api/estate'},
    {url:'https://aiderdear1.vercel.app/private-calendar-v175.js',auth:true},
    {url:'https://aiderdear1.vercel.app/AiderLog-Modern-v178-site-files.zip'},
    {url:'https://aiderdear1.vercel.app/AiderLog-v178.apk'},
    {url:'https://aiderdear1.vercel.app/AiderLog-v178.apk'},
...
test('v178 release: old PC/APK download URLs redirect directly without loops or permanent caching', () => {
  for(const edition of ['Modern','Editorial'])for(const version of [165,167,168,169,170,171,172,173,174,175,176]){
    const row=config.redirects.find(row=>row.source===`/AiderLog-${edition}-v${version}-site-files.zip`);
    assert(row,`${edition} ${version}`);assert.equal(row.destination,`/AiderLog-${edition}-v178-site-files.zip`);assert.equal(row.permanent,false);
  }
  for(const version of [165,166,167,168,169,170,171,172,173,174,175,176,177])assert(config.redirects.some(row=>row.source===`/AiderLog-v${version}.apk`),String(version));
  for(const row of config.redirects.filter(row=>row.source.endsWith('.apk'))){assert.equal(row.destination,'/AiderLog-v178.apk');assert.equal(row.permanent,false);}
...
test('v178 release: correct ZIP/APK attachment headers and private customer-share policy survive', () => {
  for(const [name,type] of [['AiderLog-Modern-v178-site-files.zip','application/zip'],['AiderLog-Editorial-v178-site-files.zip','application/zip'],['AiderLog-v178.apk','application/vnd.android.package-archive']]){
  ...
  assert(!config.headers.some(row=>/AiderLog-(?:Modern|Editorial)-v(?!178\b)\d+-site-files/.test(row.source)));
  assert(!config.headers.some(row=>/AiderLog-v(?!178\b)\d+\.apk/.test(row.source)),'Old APK redirects must not retain immutable artifact headers');
});

test('v178 release: PC packaging preserves source bytes, edition boundaries and recoverable previous archives', () => {
  assert(packaging.includes('release=178&site-edition='));assert(packaging.includes('$pcIndexText -cne $pcExpectedIndex'));assert(packaging.includes('$pcHtml -cne $pcSourceIndex'));
  ...
  assert(packaging.includes("'AiderLog-v178.apk','AiderLog-Modern-v178-site-files.zip','AiderLog-Editorial-v178-site-files.zip'"));
  assert(packaging.includes('Keep old published releases until the coordinator has passed live gates.'));
});

test('v178 release: Android includes required shared domain dependencies and no desktop-only calendar layout', () => {
...
test('v178 release: both site schedule open paths mount private date entries after the form opens',()=>{
...
test('v178 release: emotion save never creates or overwrites private-date flags; legacy originals survive emotion deletion',()=>{
...
test('v178 release: private marker clicks on the shared emotion line open Schedule, while mood clicks still open emotion detail',()=>{
