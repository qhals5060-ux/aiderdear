...
export function createDdayFirebaseFixture(){
  ...
  let beforeRead=null,failPath='',tail=Promise.resolve();
  ...
  const context={...domain,encodeStoredPayload,decodeArchive,state,auth,db:{},TextEncoder,JSON,Date,requireUser:()=>{if(!state.user)throw Error('로그인이 필요합니다.');return state.user;},cleanEmail:value=>String(value||'').trim().toLowerCase(),eventPairKey:()=>state.pair?(state.pair.memberEmails||[]).map(v=>v.toLowerCase()).sort().join('::'):'',doc:(_db,...parts)=>({path:parts.join('/')}),getDoc:read,storageStampV168:()=>({storageVersion:168}),serverTimestamp:()=>1,window:{dispatchEvent:event=>events.push(event)},CustomEvent:class{constructor(type,options){this.type=type;this.detail=options.detail;}},runTransaction:(_db,callback)=>{
  ...
  return {context,rows,reads,writes,events,state,auth,seed,pair,read:()=>context.readDdayData(),mutate:action=>context.mutateDday(action),setBeforeRead(fn){beforeRead=fn;},setFail(path){failPath=path;}};
}
