...
const release=process.env.AIDERLOG_RELEASE_VERSION||fs.readFileSync(path.join(base,'index.html'),'utf8').match(/name="aiderlog-android-build" content="v(\d+)"/)[1];
...
test('restored four-entry arc has separate 48px targets, stays within the field and follows its original guide',()=>{
  const topLeft=[[42,153],[62,90],[114,50],[180,47]],center=[176,177];
  for(let i=0;i<topLeft.length;i++){
    const[x,y]=topLeft[i];assert(x>=0&&x+48<=230&&y>=0&&y+48<=228);
    assert(Math.abs(Math.hypot(x+24-center[0],y+24-center[1])-110)<1);
  ...
  assert.doesNotMatch(css,/:has\(\[data-page="todo"\]\)/);assert.match(css,/left:66px!important;top:67px!important/);
  assert.equal(fixture().api.icon('todo'),'');
});
...
test('eleven navigation Primary tokens stay exact across system light/dark without recolouring the planet',()=>{
  ...
  for(const name of ['index.html','sw.js'])assert.ok(fs.readFileSync(path.join(base,name),'utf8').includes(`app-theme-primary-v176.css?v=${release}`));
});
