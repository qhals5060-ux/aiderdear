 ...
 try{
  for(const width of [360,384,768,904]){
   ...
   const month=await page.evaluate(()=>{
    ...
    return{
     frame:box(q('#home .schedule-dashboard-v179')),calendar:box(q('#home .schedule-calendar-v119')),side:box(q('#home .schedule-summary-v179')),todoInSide:q('[data-todo-inline-v179]').parentElement.matches('.schedule-summary-v179'),pageWidth:document.documentElement.scrollWidth,
   ...
   if(width<600){close(month.calendar.w,month.frame.w,2,'phone calendar fills width');assert(!month.todoInSide);}else{assert(month.calendar.w<month.frame.w*.75,'Fold leaves a usable second pane');assert(month.side.x>=month.calendar.x+month.calendar.w,'Fold panes do not overlap');assert(month.side.w>=222);assert(month.todoInSide,'same todo host lives in the right pane');}
