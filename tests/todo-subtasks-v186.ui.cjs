...
(async()=>{
 ...
 try{
  ...
  await page.locator('[data-todo-add-v179="todo"]').click();
  ...
  const id=created.id,row=page.locator(`[data-todo-edit-v179="${id}"].todo-copy-v179`);
  ...
  await page.locator('[data-todo-add-v179="memo"]').click();assert.equal(await sheet.locator('header button').first().textContent(),'저장');assert(await sheet.locator('header button').nth(1).getAttribute('data-todo-close-v179')!==null);assert.equal(await sheet.locator('[data-todo-subtask-add-v186]').count(),0);
