...
assert([179,180,181,182,183,184,185,186,187].includes(androidRelease),'Supported Android release metadata is required');
...
releaseTest('site and PC downloads retain their release independently of Android', () => {
  ...
  if(androidRelease>=185){assert.equal(release,184,'an app-only release must keep the restored site build');assert.match(apk,new RegExp(`versionName: 1\\.9\\.${androidRelease-110}\\s`));assert.doesNotMatch(index,/schedule-ui-v18[4567]|app-readability-v18[4567]|AiderScheduleUIBridgeV184|app-compact-v18[67]|event-routine-v18[67]|routine-ui-v187/);}
});

releaseTest('production probe checks Android v187 independently of the unchanged site and sync API',async()=>{
  const {verifyProduction}=require('../scripts/verify-production-v187.cjs');
  async function fixture({oldApk=false,advanceSite=false}={}){
    ...
    const fetchImpl=async(input,options)=>{
      ...
      if(redirect)return new Response(null,{status:307,headers:{location:oldApk&&url.pathname.endsWith('.apk')?redirect.destination.replaceAll('v187','v186'):redirect.destination}});
  ...
  const {result,requests}=await fixture();assert(result.ok,JSON.stringify(result.checks.filter(row=>!row.ok)));assert.equal(result.siteVersion,184);assert.equal(result.calendarApiVersion,184);assert.equal(result.version,187);assert.equal(result.binaryDownloadBytes,0);
