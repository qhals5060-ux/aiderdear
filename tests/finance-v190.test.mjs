...
test('finance health is read-only and API rejects missing auth and untrusted origins',async()=>{let calls=0;const handler=createHandler({getServices:async()=>{calls++;return{db:{},auth:{verifyIdToken:async()=>owner}};},dispatch:async(db,u,input)=>({ownerKey:u.uid,records:[]})});assert.equal((await http(handler,{method:'GET',url:'/?action=health',headers:{}})).body.version,190);assert.equal(calls,0);assert.equal((await http(handler,{method:'POST',headers:{}})).statusCode,401);assert.equal((await http(handler,{method:'POST',headers:{origin:'https://invalid.test',authorization:'Bearer a'}})).statusCode,403);});
function clientFixture(fetchImpl){let user={uid:'owner-a'},changed;const firebase={getState:()=>({user}),getFirebaseIdToken:async()=> 'token',subscribe(fn){changed=fn;fn();return()=>{};}};const events=new EventTarget(),client=createAssetsClient({getFirebase:()=>firebase,fetchImpl,events});return{client,firebase,change(next){user=next;changed();}};}
