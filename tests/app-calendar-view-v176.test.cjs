...
test('monthly app has 42 unique Sunday-start dates including every day in the month',()=>{const range=fixture().api.current();assert.equal(range.mode,'month');assert.equal(range.count,42);assert.equal(range.keys[0],'2026-08-30');assert.equal(range.keys[41],'2026-10-10');assert.equal(new Set(range.keys).size,42);for(let i=1;i<=30;i++)assert(range.keys.includes('2026-09-'+String(i).padStart(2,'0')));});
test('old fortnight preference and cached W calls cannot reactivate the retired view',()=>{const f=fixture('fortnight');assert.equal(f.api.current().mode,'month');assert.equal(f.api.toggle('2026-09-12').count,42);assert.equal(f.saved(),'fortnight','retired preference is not read or rewritten');const reloaded=f.window.AiderAppCalendarViewV176.create(f.storage,()=>new Date(2026,8,12));assert.equal(reloaded.current().mode,'month');});
test('month arrows cross year boundaries and return to the original month',()=>{const f=fixture();f.api.focus('2026-12-31');const before=f.api.current(),after=f.api.move(1);assert.equal(before.anchor.getMonth(),11);assert.equal(after.anchor.getFullYear(),2027);assert.equal(after.anchor.getMonth(),0);assert.equal(after.keys[0],'2026-12-27');assert.equal(f.api.move(-1).keys[0],before.keys[0]);});
...
test('denied local storage or invalid preference never blocks the monthly calendar',()=>{const window={};vm.runInNewContext(source,{window,Date});const api=window.AiderAppCalendarViewV176.create({getItem(){throw Error('blocked')},setItem(){throw Error('blocked')}});assert.equal(api.current().count,42);assert.equal(api.toggle().count,42);assert.equal(fixture('unsafe').api.current().count,42);});
...
test('actual monthly renderer shows three ordered previews, overflow and a todo slot after the second week',()=>{
  ...
  const context={window:{},Date,scheduleViewV176:f.api,scheduleSelectedV125:'2026-09-12',dateKey:d=>`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`,scheduleRowsV125:()=>rows,eventSpansDateV125:(r,k)=>r.date===k,receivedScheduleV176:()=>false,eventColorV125:()=> '#6255e8',safe:s=>String(s).replace(/</g,'&lt;').replace(/>/g,'&gt;')};
  vm.runInNewContext(fs.readFileSync(path.join(assets,'schedule-time-v179.js'),'utf8'),context);
  vm.runInNewContext(render+'; result=scheduleCellsV125(2026,8);',context);assert.equal((context.result.match(/data-schedule-date-v125=/g)||[]).length,42);assert.equal((context.result.match(/<small class="schedule-event-name-v119/g)||[]).length,3);assert.match(context.result,/schedule-more-v176"[^>]*>\+4/);assert.ok(context.result.includes('&lt;unsafe&gt;'));assert.ok(!context.result.includes('<unsafe>'));
  const todo=context.result.indexOf('data-todo-inline-v179');assert.equal((context.result.slice(0,todo).match(/data-schedule-date-v125=/g)||[]).length,14);assert.equal((context.result.match(/data-todo-inline-v179/g)||[]).length,1);
});
test('monthly pills retain time/title elements and suppress empty all-day time labels',()=>{
  const feature=fs.readFileSync(path.join(assets,'feature-system-v125.js'),'utf8'),css=fs.readFileSync(path.join(assets,'app-calendar-v179.css'),'utf8');
  assert.match(feature,/class="schedule-event-time-v176"/);assert.match(feature,/class="schedule-event-title-v176"/);
  assert.match(css,/schedule-event-title-v176\s*\{display:inline!important/);
  assert.match(css,/schedule-event-time-v176:empty\s*\{display:none!important/);
});
test('calendar header has only previous, Today and next controls, with helper loaded before renderer',()=>{
  const feature=fs.readFileSync(path.join(assets,'feature-system-v125.js'),'utf8'),html=fs.readFileSync(path.join(assets,'index.html'),'utf8'),css=fs.readFileSync(path.join(assets,'app-calendar-v179.css'),'utf8');
