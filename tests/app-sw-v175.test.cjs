...
test('Android service worker cache and shell imports use v175 and every precache file exists',()=>{
 const f=fixture();assert.equal(f.context.cacheName,'aiderlog-v175-private-calendar-insight');assert.doesNotMatch(source,/\?v=169|CACHE='aiderlog-v169/);
 ...
 for(const name of ['private-calendar-ui-v175.js','private-calendar-v175.js','friend-schedule-firebase-v175.js','business-calendar-v175.js','insight-range-v175.js','app-dday-v175.js'])assert.ok(f.context.shell.some(url=>new URL(url).pathname==='/'+name),name);
});
...
test('Android service worker never handles APIs, credentials, private files, uploads or APKs',()=>{
 const f=fixture();for(const url of ['/api/estate','/api/work','/api/private-calendar','/downloads/private.json','/private-record.json','/customer-contract.pdf','/AiderLog-v175.apk'])assert.equal(f.request(url).response,undefined,url);
 assert.equal(f.request('/firebase-app.js?v=175',{headers:new Headers({Authorization:'Bearer local-test'})}).response,undefined);
...
test('Android static calendar code can still use the declared offline cache',async()=>{
 const f=fixture(),r=f.request('/private-calendar-v175.js?v=175');assert.ok(r.response);await r.response;await Promise.all(r.waits);assert.equal(f.stored.length,1);
});
