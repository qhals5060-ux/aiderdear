...
test('todo strip follows the currently shown current-week row, not a fixed second week',()=>{
  ...
  for(const [today,last] of [['2026-09-01',6],['2026-09-06',13],['2026-09-19',20],['2026-09-30',34],['2026-10-10',41]]){
    ...
    assert.equal(result.rows.split('66px').length,2);assert.equal((result.rows.match(/minmax/g)||[]).length,6);
  }
...
test('opening and closing Fold retains the same inline todo host without losing its contents',()=>{
  ...
  h.wide.matches=true;h.api.refresh();assert.equal(todo.parentNode,days);assert.equal(side.children.length,0);
...
test('empty todo strip is retained and the compact Fold overrides fill the calendar width',()=>{
  ...
  const compact=fs.readFileSync(path.join(root,'android-src/assets/app-compact-v186.css'),'utf8');assert.match(compact,/schedule-dashboard-v179\{grid-template-columns:minmax\(0,1fr\)/);
...
test('app calendar changes mount without adding app CSS to the website',()=>{
  for(const file of ['app-calendar-v179.css','app-calendar-v179.js'])assert(fs.readFileSync(path.join(root,'android-src/assets/index.html'),'utf8').includes(file));
