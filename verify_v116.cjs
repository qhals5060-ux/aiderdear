...
for (const marker of [
  ...
  if (!template.includes(marker)) throw new Error(`Missing v116 template marker: ${marker}`);
}
...
for (const marker of [
  ...
  if (!engine.includes(marker)) throw new Error(`Missing v116 engine marker: ${marker}`);
}
for (const marker of ['AiderLog v115', '--readable-min:11px', 'grid-template-rows:repeat(5,minmax(58px,1fr))']) {
  if (!style.includes(marker)) throw new Error(`Missing inherited readable Language Lab marker: ${marker}`);
}

for (const marker of [
  'content="v116"',
  'language-lab-v18-engine.js?v=116',
  'language-lab-v18.js?v=116',
  "const PAPER_ROOT = '#paperStage'",
  'size < floor',
  "classList.add('font-floor-raised')",
  'AiderLog v116 · typography-safe interior reflow',
  '#stage .cal-top{height:auto!important',
  '#privateStage :is(.routine-left-v106,.routine-right-v106){overflow:auto!important',
  '#eventStage .event-index-card{height:auto!important',
  '#personalStage .workflow-dashboard-v103{grid-template-rows:auto auto auto minmax(0,1fr)!important',
  '#taskStage .task-page.current{overflow:auto!important',
  '.nav-row{grid-row:2!important}',
]) {
  if (!html.includes(marker)) throw new Error(`Missing v116 HTML marker: ${marker}`);
}
for (const marker of ['.logo small{display:none!important}', '#privateStage .routine-mandala-v104>header span{display:none!important}']) {
  if (!html.includes(marker)) throw new Error(`Missing compact-copy override: ${marker}`);
}
if (!component.includes('language-lab-v18-template.html?v=116') || !component.includes('language-lab-v18.css?v=116')) {
  throw new Error('Language Lab assets are not cache-busted to v116.');
}
if (pkg.name !== 'aiderlog-v116' || pkg.version !== '116.0.0') throw new Error('Unexpected package version.');
if (!serviceWorker.startsWith("const CACHE='aiderlog-v116-readable-reflow';")) {
  throw new Error('Unexpected v116 service worker cache key.');
}
...
console.log('v116 readable non-PAPER reflow and ten-lesson curriculum: ok');
