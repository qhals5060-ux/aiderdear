(() => {
  ...
  function range(){const native=[...document.querySelectorAll('#home [data-schedule-date-v125][data-date]')],dates=(native.length?native:[...document.querySelectorAll('#calendar .day[data-date]')]).map(el=>el.dataset.date).sort();return dates.length?{from:dates[0],to:dates.at(-1)}:null;}
  function install(){if(!box?.isConnected){const native=document.querySelector('.schedule-dialog-v125 [data-app-friend-slot-v179]'),owner=native||$('eventOwnerField');if(owner){box=document.createElement('fieldset');box.id=native?'appEventFriendShareV179':'eventFriendShareV175';box.className='field full';box.hidden=true;owner.after(box);}}
