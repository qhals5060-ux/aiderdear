...
(() => {
  ...
  function labNotebookView() {
    const links = labLinks.map((row,index) => {
      ...
      const researcher = row.researcherName || `연구원 ${index+1}`;
      return `<article class="site-lab-link-v158 ${activeLabToken===row.token?'active':''}"><span><small>${esc(row.label || `LAB NOTE ${index+1}`)}</small><b>${esc(researcher)}</b></span><input readonly value="${esc(url)}" aria-label="${esc(researcher)} 고정 실험노트 링크"><div class="site-lab-link-actions-v160"><button type="button" data-lab-view-token="${esc(row.token)}">노트 보기</button><button type="button" data-lab-copy="${esc(url)}">복사</button><button type="button" data-lab-share="${esc(url)}" data-lab-share-name="${esc(researcher)}">공유</button></div></article>`;
    }).join('');
    const activeLink=labLinks.find(row=>row.token===activeLabToken),visibleEntries=activeLabToken==='all'?labEntries:labEntries.filter(row=>row.token===activeLabToken);
    const filters=`<div class="site-lab-filters-v160"><button type="button" class="${activeLabToken==='all'?'active':''}" data-lab-view-token="all">전체 <b>${labEntries.length}</b></button>${labLinks.map((row,index)=>`<button type="button" class="${activeLabToken===row.token?'active':''}" data-lab-view-token="${esc(row.token)}">${esc(row.researcherName||`연구원 ${index+1}`)} <b>${labEntries.filter(entry=>entry.token===row.token).length}</b></button>`).join('')}</div>`;
    const notes = visibleEntries.map(row => `<article class="site-lab-note-v158"><header><span><small>${esc(row.researcher || row.linkLabel || '연구원')}</small><b>${esc(row.title || '실험 기록')}</b></span><time>${esc(String(row.performedAt || '').replace('T',' '))}</time></header><dl><div><dt>프로젝트 · 코드</dt><dd>${esc([row.project,row.code].filter(Boolean).join(' · ') || '미입력')}</dd></div><div><dt>프로토콜 · Batch</dt><dd>${esc([row.protocol,row.batch].filter(Boolean).join(' · ') || '미입력')}</dd></div><div><dt>QC</dt><dd>${esc(row.qcResult || '확인 전')} · ${esc(row.qcCriteria || '기준 미입력')}</dd></div><div><dt>다음 행동</dt><dd>${esc(row.nextAction || '대표 확인')}</dd></div></dl><details><summary>실험노트 한눈에 보기</summary><p><b>목적</b>${esc(row.purpose || '—')}</p><p><b>재료·시료·장비</b>${esc(row.materials || '—')}</p><p><b>절차</b>${esc(row.procedure || '—')}</p><p><b>결과·관찰</b>${esc(row.result || '—')}</p></details>${(row.media||[]).length?`<footer>${row.media.map(media=>`<button type="button" data-lab-media="${esc(row.token)}|${esc(row.id)}|${esc(media.id)}">${/^video\//.test(media.type)?'영상':'사진'} · ${esc(media.name)}</button>`).join('')}</footer>`:''}</article>`).join('');
    return `<section class="site-work-lab-v158"><header class="site-work-section-head-v146"><div><small>EXPERIMENT NOTEBOOK</small><h3>${activeLink?`${esc(activeLink.researcherName||'연구원')} 실험노트`:'연구원별 실험노트'}</h3><p>5개의 고정 링크를 연구원별로 공유하고, 기록은 노트별로 따로 확인할 수 있습니다.</p></div><button class="site-work-button-v146" type="button" data-lab-refresh>새 기록 확인</button></header><div class="site-lab-links-v158">${links || '<div class="site-work-empty-v146">로그인 후 연구원 링크를 준비합니다.</div>'}</div>${filters}<div class="site-lab-notes-v158">${notes || '<div class="site-work-empty-v146">선택한 연구원의 실험노트가 없습니다.</div>'}</div></section>`;
  }
  ...
  async function action(event) {
    ...
    const viewToken = event.target.closest('[data-lab-view-token]'); if (viewToken) { activeLabToken=viewToken.dataset.labViewToken||'all'; render(); requestAnimationFrame(()=>stage.querySelector('.site-lab-notes-v158')?.scrollIntoView({block:'start',behavior:'smooth'})); return; }
    const copy = event.target.closest('[data-lab-copy]'); if (copy) { await navigator.clipboard.writeText(copy.dataset.labCopy); copy.textContent='복사됨'; setTimeout(()=>copy.textContent='복사',1200); return; }
    const share = event.target.closest('[data-lab-share]'); if (share) { const url=share.dataset.labShare,name=share.dataset.labShareName||'연구원'; try { if(navigator.share)await navigator.share({title:`${name} AiderLog 실험노트`,text:`${name} 전용 실험노트 입력 링크입니다.`,url});else {await navigator.clipboard.writeText(url);share.textContent='복사됨';setTimeout(()=>share.textContent='공유',1200)}} catch(error){if(error?.name!=='AbortError')alert('링크를 공유하지 못했습니다.')} return; }
