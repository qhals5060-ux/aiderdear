...
// Usage: node verify-production-v190.cjs --url https://your-production-host
const RELEASE_ROOT = 'https://github.com/qhals5060-ux/aiderdear/releases/download/';
const ASSETS = [{file:'AiderLog-v190.apk',version:190},{file:'AiderLog-v190-site-files.zip',version:190}];
...
async function verifyProduction(input, {fetchImpl = fetch, checkReleaseAssets = true} = {}) {
  ...
  const request = (url, method = 'GET', redirect = 'manual') => fetchImpl(url, {
    method, redirect, headers: {'Cache-Control': 'no-cache', 'User-Agent': 'AiderLog-v190-readonly-verification'},
  ...
  await record('site build and module references', async () => {
    ...
    if (!/<meta\b[^>]*name=["']aiderlog-android-build["'][^>]*content=["']v190["']/i.test(text)) throw new Error('Android v190 metadata is missing');
    ...
    return {build:'v190',androidBuild:'v190',siteDesign:'Modern; Event/Routine refined'};
  });
  for (const [path, marker] of [
    ['/calendar-sync-v184.js?v=184', 'createCalendarSyncClient'],
    ['/firebase-app.js?v=190', "from './calendar-sync-v184.js'"],
    ['/sw.js?verify=v190', 'aiderlog-v190-site-modern'],
  ]) {
  ...
  await record('Finance workspace API health',async()=>{
    ...
    if(health.ok!==true||health.version!==190||health.requiresAuth!==true||health.livePrices!==false||response.headers.get('x-aiderlog-finance')!=='190')throw Error('Finance API health mismatch');
    return {version:190,requiresAuth:true,livePrices:false};
  });
  await record('Finance workspace rejects unauthenticated reads',async()=>{
    const response=await request(new URL('/api/assets',base),'POST');await response.body?.cancel();if(response.status!==401)throw Error('Expected 401');return{status:401};
  });
  for(const [file,marker]of [['finance-v190.html','finance-ui-v190.js'],['site-investment-v190.js','investment'],['assets-client-v190.js','AiderAssetsBridgeV184'],['finance-ui-v190.js','예시']])await record('Finance public asset '+file,async()=>{const {text}=await fetchText('/'+file+'?v=190');if(!text.includes(marker))throw Error('Missing finance module marker');return{bytes:Buffer.byteLength(text)};});
  for (const file of [...ASSETS.map(asset=>asset.file), 'AiderLog-v189.apk', 'AiderLog-v189-site-files.zip', 'AiderLog-v188.apk', 'AiderLog-v187.apk', 'AiderLog-v186.apk', 'AiderLog-v185.apk', 'AiderLog-v184.apk', 'AiderLog-v183.apk', 'AiderLog-Editorial-v184-site-files.zip', 'AiderLog-Modern-v184-site-files.zip']) {
  ...
  return {url: base.href, version:190, siteVersion:190, calendarApiVersion:184, readOnly: true, binaryDownloadBytes: 0, ok: checks.every(row => row.ok), checks};
}
...
if (require.main === module) {
  ...
  if (args.length !== 2 || args[0] !== '--url') {
    console.error('Usage: node verify-production-v190.cjs --url https://your-production-host');
    process.exitCode = 2;
