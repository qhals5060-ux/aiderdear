...
// Usage: node verify-production-v189.cjs --url https://your-production-host
const RELEASE_ROOT = 'https://github.com/qhals5060-ux/aiderdear/releases/download/';
const ASSETS = [{file:'AiderLog-v189.apk',version:189},{file:'AiderLog-v189-site-files.zip',version:189}];
...
async function verifyProduction(input, {fetchImpl = fetch, checkReleaseAssets = true} = {}) {
  ...
  const request = (url, method = 'GET', redirect = 'manual') => fetchImpl(url, {
    method, redirect, headers: {'Cache-Control': 'no-cache', 'User-Agent': 'AiderLog-v189-readonly-verification'},
  ...
  await record('site build and module references', async () => {
    const {text} = await fetchText('/?verify=android-v189');
    if (!/<meta\b[^>]*name=["']aiderlog-build["'][^>]*content=["']v189["']/i.test(text)) throw new Error('aiderlog-build v189 metadata is missing');
    ...
    if (!/<meta\b[^>]*name=["']aiderlog-android-build["'][^>]*content=["']v189["']/i.test(text)) throw new Error('Android v189 metadata is missing');
    ...
    return {build:'v189',androidBuild:'v189',siteDesign:'Modern; Event/Routine refined'};
  });
  for (const [path, marker] of [
    ['/calendar-sync-v184.js?v=184', 'createCalendarSyncClient'],
    ['/firebase-app.js?v=184', "from './calendar-sync-v184.js'"],
    ['/sw.js?verify=v189', 'aiderlog-v189-site-modern'],
  ]) {
  ...
  for (const file of [...ASSETS.map(asset=>asset.file), 'AiderLog-v188.apk', 'AiderLog-v187.apk', 'AiderLog-v186.apk', 'AiderLog-v185.apk', 'AiderLog-v184.apk', 'AiderLog-v183.apk', 'AiderLog-Editorial-v184-site-files.zip', 'AiderLog-Modern-v184-site-files.zip']) {
  ...
  return {url: base.href, version:189, siteVersion:189, calendarApiVersion:184, readOnly: true, binaryDownloadBytes: 0, ok: checks.every(row => row.ok), checks};
}
...
if (require.main === module) {
  ...
  if (args.length !== 2 || args[0] !== '--url') {
    console.error('Usage: node verify-production-v189.cjs --url https://your-production-host');
    process.exitCode = 2;
