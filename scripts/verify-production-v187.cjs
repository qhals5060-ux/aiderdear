...
// Usage: node verify-production-v187.cjs --url https://your-production-host
const RELEASE_ROOT = 'https://github.com/qhals5060-ux/aiderdear/releases/download/';
const ASSETS = [{file:'AiderLog-v187.apk',version:187},{file:'AiderLog-Editorial-v184-site-files.zip',version:184},{file:'AiderLog-Modern-v184-site-files.zip',version:184}];
...
async function verifyProduction(input, {fetchImpl = fetch, checkReleaseAssets = true} = {}) {
  ...
  const request = (url, method = 'GET', redirect = 'manual') => fetchImpl(url, {
    method, redirect, headers: {'Cache-Control': 'no-cache', 'User-Agent': 'AiderLog-v187-readonly-verification'},
  ...
  await record('site build and module references', async () => {
    const {text} = await fetchText('/?verify=android-v187');
    ...
    if (/schedule-ui-v184|AiderScheduleUIBridgeV184|app-compact-v18[67]|event-routine-v18[67]|routine-ui-v187/.test(text)) throw new Error('App-only layout must not be mounted on the site');
    if (!/<meta\b[^>]*name=["']aiderlog-android-build["'][^>]*content=["']v187["']/i.test(text)) throw new Error('Android v187 metadata is missing');
    for(const asset of ASSETS)if(!text.includes(`href="./${asset.file}"`))throw new Error(`Missing download: ${asset.file}`);
    return {build:'v184',androidBuild:'v187',siteDesign:'v183 restored'};
  });
  ...
  for (const file of [...ASSETS.map(asset=>asset.file), 'AiderLog-v186.apk', 'AiderLog-v185.apk', 'AiderLog-v184.apk', 'AiderLog-v183.apk']) {
  ...
  return {url: base.href, version:187, siteVersion:184, calendarApiVersion:184, readOnly: true, binaryDownloadBytes: 0, ok: checks.every(row => row.ok), checks};
}
...
if (require.main === module) {
  ...
  if (args.length !== 2 || args[0] !== '--url') {
    console.error('Usage: node verify-production-v187.cjs --url https://your-production-host');
    process.exitCode = 2;
