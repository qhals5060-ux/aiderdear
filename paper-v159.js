...
(async function(){
  ...
  const style=document.createElement('link');style.rel='stylesheet';style.href='./paper-v159.css?v=167';root.append(style);
  const nav=$('#paperNav');
  nav.innerHTML=`
  ...
  if(oldGuide){
    ...
    homeButton.innerHTML='<svg aria-hidden="true" viewBox="0 0 24 24" width="19" height="19" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="m3 10 9-7 9 7v10a1 1 0 0 1-1 1h-5v-7H9v7H4a1 1 0 0 1-1-1Z"/></svg>';
    homeButton.setAttribute('aria-label','Paper 연구 홈 열기');
    homeButton.setAttribute('title','연구 홈');
    homeButton.setAttribute('aria-current',workspace.currentView==='hub'?'page':'false');
    oldGuide.replaceWith(homeButton);
    homeButton.addEventListener('click',()=>workspace.openHome());
  }
  nav.querySelector(`[data-view="${workspace.currentView==='paper'?'library':workspace.currentView}"]`)?.classList.add('active');
  const snap=()=>window.AiderPaperBridge?.snapshot?.()||{paperItems:[],researchInsights:[],researchNotes:[],researchIdeas:[],researchDesigns:[]};
  function heading(kicker,title){return `<header class="v159-heading"><div><span>${esc(kicker)}</span><h1>${esc(title)}</h1></div></header>`}
