...
// Dormant template: re-enabling data access requires an explicit code change.
export function createHandler({enabled=false,getServices=async()=>{const [{services},{FieldValue}]=await Promise.all([import('../server/firebase-admin.mjs'),import('firebase-admin/firestore')]);return{...services(),commitTimestamp:()=>FieldValue.serverTimestamp()};},dispatch=financeDispatch}={}){return async(req,res)=>{
 ...
 try{
  if(req.method==='GET'&&new URL(req.url,'https://request.invalid').searchParams.get('action')==='health'){res.statusCode=200;return res.end(JSON.stringify({ok:true,version:190,mode:enabled?'active':'template',requiresAuth:enabled,dataAccess:enabled,livePrices:false}));}
  if(!enabled){res.statusCode=423;return res.end(JSON.stringify({mode:'template',dataAccess:false,error:'재테크는 미리보기입니다. 저장과 데이터 연결을 사용하지 않습니다.'}));}
