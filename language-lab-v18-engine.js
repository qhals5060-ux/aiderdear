...
function buildConversationFormula(base, language, situation, mode, levelIndex) {
  ...
  const chunkByMode = {
    ...
    ja: { social: "ちなみに", service: "お願いできますか", professional: "念のため確認", resolution: "何とかできますか", business: "認識を合わせる" }}[language][mode];
  return {
    formula,
...
function buildVerifiedPronunciationFlow(base, language, mode, levelIndex) {
  ...
  const closers = {
    ...
    ja: ["もう一度お願いします。", "念のため、これで合っていますか。", "ここではどのように使うのか教えていただけますか。", "これが最終決定という理解でよろしいでしょうか。", "ニュアンスまで含めて、私の理解が正しいか確認させてください。"]}[language];
...
function createDialogueSuggestionPicker(language, target = "") {
  const fallbacks = {
    ...
    ja: ["もう少し詳しく教えていただけますか。", "念のため確認させてください。", "どの方法がおすすめですか。", "その方法でお願いします。", "内容を確認してもよろしいですか。", "説明していただきありがとうございます。"]}[language];
  const used = new Set();
  return (candidates) => {
    const result = [];
...
function courseScriptGuide(language, levelIndex, anchorIndex, source) {
  if (source.reading || source.romanization) {
    return {
      label: language === "ja" ? "문자·로마자" : "리듬 단위",
...
const state = {
  language: ["ja", "en"].includes(saved.language) ? saved.language : "ja",
  levelByLanguage: {
...
function pronunciationButtons(text, language) {
  const variants = {
    ...
    ja: [{ label: "일본 표준어", locale: "ja-JP" }]}[language];
