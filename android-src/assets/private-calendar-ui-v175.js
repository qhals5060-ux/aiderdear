import {canUsePrivateIntimacy, privateCalendarMarkers, privateCalendarDayState, privateCalendarDate} from './private-calendar-v175.js';

// Sensitive dates stay in account/couple-bound memory, never in ordinary shared
// schedules, URL parameters, analytics or browser persistent storage.
...
let pairWatchKey='',stopPairWatch=null;
const dateNow=()=>new Date(Date.now()+9*3600000).toISOString().slice(0,10);
function identity(){const u=user(),key=u?.uid?`${u.uid}:${u.email}:${u.emailVerified===true}:${api()?.getState?.().pair?.id||''}`:'';if(key!==actor){actor=key;epoch++;data=null;pending=null;rangeKey='';retryAt=0;error='';draft=null;editing=null;entryDirty=false;settingsDirty=false;entryDate='';scheduleReturn=null;busy=false;dialog?.close();clearMarkers();if(dialog)dialog.remove();dialog=null;}return key;}
...
function installScheduleEntries(){
  for(const host of document.querySelectorAll('#scheduleModal, .schedule-dialog-v125')){
    ...
    for(const kind of ['period',...(canIntimacy()?['intimacy']:[])]){
      ...
      let selected=false;try{selected=privateCalendarDayState(data,kind,context.date.value).active;}catch{}
...
async function openFromSchedule(host,kind){
  ...
  try{
    const snapshot=await api().readPrivateCalendarData({from:date,to:date});
    if(key!==identity()||version!==epoch)return;
    if(kind==='intimacy'&&(!canUsePrivateIntimacy(user())||!snapshot.canUseIntimacy))return;
    const current=privateCalendarDayState(snapshot,kind,date);
    await api().mutatePrivateCalendar({type:'day-set',kind,date,active:!current.active,expectedRevision:current.revision});
    if(key!==identity()||version!==epoch)return;
    error='';
  }catch(err){if(key===identity()&&version===epoch){message(failure(err));const node=document.createElement('span');node.setAttribute('role','status');node.dataset.privateDayErrorV178='';node.textContent=error;context.footer.querySelector('[data-private-day-error-v178]')?.remove();context.footer.appendChild(node);}}
  finally{if(version===epoch){busy=false;installScheduleEntries();}}
  if(key===identity()&&version===epoch&&!error){context.footer.querySelector('[data-private-day-error-v178]')?.remove();await refresh(true);installScheduleEntries();}
}
...
function build(){if(dialog)return;dialog=document.createElement('dialog');dialog.id='privateCalendarDialogV175';dialog.setAttribute('aria-labelledby','privateCalendarTitleV175');
  ...
  dialog.querySelector('[data-private-retry]').addEventListener('click',()=>refresh(true));$('privateCalendarSettingsV175').addEventListener('input',()=>{settingsDirty=true;});
...
function renderStatus(){if(!dialog)return;$('privateCalendarStatusV175').textContent=error;dialog.querySelectorAll('input,button').forEach(el=>el.disabled=busy);dialog.querySelector('[data-private-retry]').hidden=!error;}
...
['aiderdear-firebase-ready','aiderdear-firebase-state'].forEach(name=>addEventListener(name,()=>{identity();install();refresh();watchPair();}));
addEventListener('hashchange',watchPair);
addEventListener('focus',()=>refresh(true));
document.addEventListener?.('visibilitychange',()=>{watchPair();if(document.visibilityState==='visible')refresh(true);});
if(api()?.subscribe)api().subscribe(()=>{identity();install();refresh();watchPair();});calendarChanged();
