...
(function(){
  ...
  window.initAiderLogLanguageLab=async function(root,shell){
    try{
      ...
      const outerHead=host?.closest?.('#language')?.querySelector?.('.language-edu-head');
      const innerLanguage=root?.querySelector?.('#language-select');
      const innerLevel=root?.querySelector?.('#level-select');
      if(outerHead&&innerLanguage&&innerLevel&&!outerHead.querySelector('.language-quick-select-v149')){
        const controls=document.createElement('div');
        controls.className='language-quick-select-v149';
        controls.innerHTML='<label><span>언어</span><select aria-label="학습 언어 빠른 선택"></select></label><label><span>난이도</span><select aria-label="학습 난이도 빠른 선택"></select></label>';
        const languageSelect=controls.querySelector('label:first-child select');
        const levelSelect=controls.querySelector('label:last-child select');
        const copyOptions=(source,target)=>{target.innerHTML=[...source.options].map(option=>`<option value="${option.value}">${option.textContent}</option>`).join('');target.value=source.value};
        const syncOuter=()=>{copyOptions(innerLanguage,languageSelect);copyOptions(innerLevel,levelSelect)};
        const syncInner=async(source,target)=>{target.value=source.value;target.dispatchEvent(new Event('change',{bubbles:true}));await new Promise(resolve=>setTimeout(resolve,30));syncOuter()};
        syncOuter();
        languageSelect.addEventListener('change',()=>syncInner(languageSelect,innerLanguage));
        levelSelect.addEventListener('change',()=>syncInner(levelSelect,innerLevel));
        innerLanguage.addEventListener('change',()=>setTimeout(syncOuter,60));
        innerLevel.addEventListener('change',()=>setTimeout(syncOuter,60));
        outerHead.querySelector('h1')?.after(controls);
      }
      const style=document.createElement('style');
      style.dataset.languageV149='true';
      style.textContent=`
        .app-header{height:44px!important;min-height:44px!important;padding:4px 8px!important;display:flex!important;align-items:center!important;justify-content:flex-end!important;gap:6px!important}
        ...
        .header-course{display:none!important}.header-course .course-selectors{display:flex!important;align-items:center!important;justify-content:flex-end!important;gap:4px!important;width:auto!important}
        ...
        .single-page{height:calc(100% - 44px)!important;min-height:0!important;padding:8px!important;overflow-y:auto!important}.dashboard-section{margin-top:10px!important}.course-panel{min-height:360px!important}
      ...
      `;
      if (!host?.hasAttribute('data-training-v166')) root.append(style);
      return result;
    }catch(error){
