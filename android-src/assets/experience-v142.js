(() => {
  ...
  function textTargets(root){
    ...
    while((node=walker.nextNode())){
      ...
      if(parent.matches('.schedule-holiday-v144'))continue;
      result.add(parent);
    }
  ...
  const mailPhotosV176=window.AiderLogPhotosV176.createBatch({owner:()=>String(firebaseState().user?.uid||''),limit:6});
  function renderMailPhotosV176(form){window.AiderLogPhotosV176.mountPreview(form?.querySelector('[data-mail-preview-v176]'),mailPhotosV176);}
  ...
  function ensureMailbox(){
    ...
    overlay.addEventListener('click',async event=>{
      ...
      const letter=event.target.closest('[data-letter-v142]');if(letter){const row=(firebaseState().directLetters||[]).find(item=>item.id===letter.dataset.letterV142),urls=row?.photoDataUrls?.length?row.photoDataUrls:(row?.photoDataUrl?[row.photoDataUrl]:[]);if(urls.length)window.AiderLogPhotosV176?.gallery(urls.map(localImage=>({localImage})),async()=>'', '편지 사진');if(letter.classList.contains('unread')){try{await firebase()?.markDirectLetterRead?.(letter.dataset.letterV142)}catch{}renderMailbox()}}
    });
    return overlay;
  }
  function renderMailbox(){
    ...
    host.innerHTML=`${people.length?`<form class="utility-compose-v142" data-mail-form-v142><select name="recipient" aria-label="받는 사람">${people.map(row=>`<option value="${safe(row.uid)}">${safe(row.kind)} · ${safe(row.name||row.email)}</option>`).join('')}</select><span></span><button type="submit">보내기</button><textarea name="body" maxlength="800" placeholder="마음을 담은 우편을 적어보세요" required></textarea><label class="mail-photo-pick-v176">사진 여러 장<input type="file" accept="image/*" multiple data-mail-photo-v176></label><div data-mail-preview-v176 class="photo-batch-v176"></div></form>`:'<div class="utility-empty-v142" style="min-height:90px">개인 페이지에서 커플이나 친구를 연결하면 우편을 보낼 수 있어요.</div>'}<nav class="mail-tabs-v142"><button type="button" data-mail-tab-v142="received" class="${mailTab==='received'?'active':''}">받은 우편</button><button type="button" data-mail-tab-v142="sent" class="${mailTab==='sent'?'active':''}">보낸 우편</button></nav><div class="utility-list-v142">${rows.length?rows.map(row=>{const incoming=String(row.toUid||'')===uid,unread=incoming&&(!Array.isArray(row.readBy)||!(row.readBy||[]).includes(uid));const who=incoming?(row.fromName||row.fromEmail||'보낸 사람'):(row.toName||row.toEmail||'받는 사람');return `<button type="button" class="mail-row-v142 ${unread?'unread':''}" data-letter-v142="${safe(row.id)}"><span><b>${incoming?'From.':'To.'} ${safe(who)}</b><p>${safe(row.body||'')}</p>${(row.photoDataUrls?.length||row.photoDataUrl)?`<small>▧ 사진 ${row.photoDataUrls?.length||1}장</small>`:''}</span><time>${safe(letterTime(row.createdAt))}</time></button>`}).join(''):'<div class="utility-empty-v142">아직 우편이 없습니다.</div>'}</div>`;
    const form=$('[data-mail-form-v142]',host);if(form){renderMailPhotosV176(form);form.querySelector('[data-mail-photo-v176]').onchange=event=>{const errors=mailPhotosV176.add(Array.from(event.target.files||[]));event.target.value='';renderMailPhotosV176(form);if(errors.length)alert(errors.join('\n'));};}if(form)form.addEventListener('submit',async event=>{
      ...
      try{if(!firebase()?.sendDirectLetter)throw Error('우편 연결을 확인해주세요.');if(!mailPhotosV176.count)mailPhotosV176.reset();const actor=firebaseState().user?.uid,photos=await mailPhotosV176.upload(async file=>({name:file.name,localImage:await window.AiderLogPhotosV176.dataUrl(await window.AiderLogPhotosV176.compressPhoto(file,{max:900,budget:100000}))}),()=>renderMailPhotosV176(form));if(actor!==firebaseState().user?.uid)throw Error('계정이 변경되었습니다.');await firebase().sendDirectLetter({toUid:person.uid,toEmail:person.email,toName:person.name||person.email,body,photoDataUrls:photos.map(photo=>photo.localImage)});if(actor!==firebaseState().user?.uid)throw Error('계정이 변경되었습니다.');mailPhotosV176.committed();mailPhotosV176.reset();form.elements.body.value='';mailTab='sent';renderMailbox()}catch(error){alert(error?.message||'우편을 보내지 못했습니다.')}finally{if(button.isConnected){button.disabled=false;button.textContent='보내기';renderMailPhotosV176(form)}}
    });
  }
  ...
  function navigateWheelHome(){setWheelOpen(false);if(typeof window.go==='function')window.go('home',false);else location.hash='home'}
  function installWheelControl(){
    const wheel=$('#wheel'),core=$('#wheelCore');if(!wheel||!core||wheel.dataset.controlV142==='1')return;
    /* v142 can initialize before the later compatibility layer. Mark both
       controllers as owned so only this deterministic gesture path binds. */
    wheel.dataset.controlV142='1';wheel.dataset.controlV143='1';
    document.addEventListener('pointerdown',event=>{
      ...
      const wasOpen=wheel.classList.contains('open');
      wheelGesture={id:event.pointerId,wasOpen,startX:event.clientX,startY:event.clientY,selected:item||null,core:!!pressedCore,long:!!item};
      clearTimeout(wheelHoldTimer);
      if(item){setWheelOpen(true);item.classList.add('hovered');item.dataset.wheelSelectedV142='true'}else{
        clearWheelSelection();core.classList.add('pressing');
        wheelHoldTimer=setTimeout(()=>{if(!wheelGesture||wheelGesture.id!==event.pointerId)return;wheelGesture.long=true;setWheelOpen(true);navigator.vibrate?.(7)},420);
      }
    ...
    document.addEventListener('pointermove',event=>{
      ...
      const moved=Math.hypot(event.clientX-wheelGesture.startX,event.clientY-wheelGesture.startY);
      if(moved>18&&!wheelGesture.long){clearTimeout(wheelHoldTimer);wheelGesture.long=true;setWheelOpen(true)}
      if(wheelGesture.long)nearestWheelItem(event.clientX,event.clientY);
    },true);
    const finish=event=>{
      if(!wheelGesture||event.pointerId!==wheelGesture.id)return;event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();
      clearTimeout(wheelHoldTimer);wheelHoldTimer=null;core.classList.remove('pressing');
      const state=wheelGesture,moved=Math.hypot(event.clientX-state.startX,event.clientY-state.startY);let selected=state.selected;
      if(state.long&&moved>12)selected=nearestWheelItem(event.clientX,event.clientY);wheelGesture=null;ignoreWheelClickUntil=Date.now()+360;
      try{core.releasePointerCapture?.(event.pointerId)}catch{}
      if(selected&&(!state.core||state.long)){navigateWheel(selected);return}
      clearWheelSelection();setWheelOpen(false);if(state.core&&!state.long)navigateWheelHome();
    };
    document.addEventListener('pointerup',finish,true);
    document.addEventListener('pointercancel',event=>{if(!wheelGesture||event.pointerId!==wheelGesture.id)return;event.preventDefault();event.stopImmediatePropagation();clearTimeout(wheelHoldTimer);wheelHoldTimer=null;core.classList.remove('pressing');wheelGesture=null;clearWheelSelection();setWheelOpen(false)},true);
    document.addEventListener('click',event=>{
      const item=event.target.closest?.('.global-wheel-item-v126'),pressedCore=event.target.closest?.('#wheelCore');if(!item&&!pressedCore)return;
      event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();
      if(Date.now()<ignoreWheelClickUntil)return;
      if(item)navigateWheel(item);else navigateWheelHome();
    },true);
    core.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();navigateWheelHome()}else if(event.key==='ArrowUp'){event.preventDefault();setWheelOpen(true)}else if(event.key==='Escape'){setWheelOpen(false)}});
  }
