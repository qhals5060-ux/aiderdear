(function () {
  ...
  function scheduleCellsV125(year,month) {
    ...
    return Array.from({length:42},(_,index) => {
      ...
      return `<button type="button" class="day schedule-day-v119${outside?' outside':''}${key===scheduleSelectedV125?' selected':''}${key===dateKey(new Date())?' today':''}" data-schedule-date-v125="${key}" data-date="${key}" aria-label="${key} 일정 관리"><span class="schedule-day-number-v119">${date.getDate()}</span><span class="calendar-status-icons" aria-label="날짜 기록"></span>${rows[0]?`<small class="schedule-event-name-v119">${safe(rows[0].allDay?'종일':rows[0].time||'')} ${safe(rows[0].title||'일정')}</small>`:''}<span class="schedule-event-dots-v119">${rows.slice(0,3).map(row=>`<i style="--event-color:${safe(eventColorV125(row))}"></i>`).join('')}</span></button>`;
    }).join('');
  }
