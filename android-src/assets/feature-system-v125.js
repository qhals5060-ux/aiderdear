(function () {
  ...
  function applyFixedWheelV125() {
    ...
    const order=['fifth','personal','routine','event','todo'],labels={event:'Event',routine:'Routine',personal:'Personal',fifth:'My',todo:'Todo'},icons={event:'event',routine:'routine',personal:'profile',fifth:'my',todo:'task'};
  ...
  function scheduleUpcomingV125() {
    ...
    return `<section class="schedule-upcoming-v179" aria-label="가까운 일정">${time.entries(rows).map(({row,separatorBefore})=>`${separatorBefore?'<span class="upcoming-halfday-v179" aria-label="오후 일정">-</span>':''}<button class="schedule-upcoming-line-v179${receivedScheduleV176(row)?' schedule-received-v176':''}" type="button" data-schedule-edit-v125="${safe(row.id)}" style="--owner-color:${safe(ownerColor(row))}" title="${safe(row.date||'')} · ${safe(row.title||'일정')}"><i aria-hidden="true"></i><span>${row.allDay?'':`<time>${safe(time.format(row)||'')}</time>`}<b>${safe(row.title||'일정')}</b></span></button>`).join('')}</section>`;
  }
  ...
  function ensureScheduleDialogV125() {
    ...
    overlay=document.createElement('div');overlay.className='schedule-dialog-v125';overlay.innerHTML=`<section role="dialog" aria-modal="true"><header class="feature-dialog-head-v125"><h2 data-schedule-title-v125>일정 추가</h2><div class="schedule-editor-tools-v179" data-private-header-v179><div class="schedule-editor-primary-v179"><button type="button" data-schedule-dialog-close-v125 aria-label="닫기">${icon('close')}</button><button type="submit" form="appScheduleFormV179" class="primary">저장</button></div></div></header><div class="schedule-dialog-body-v125"><div class="schedule-day-list-v125" data-schedule-day-list-v125></div><form id="appScheduleFormV179" class="schedule-form-v125" data-schedule-form-v125><input type="hidden" name="id"><div class="schedule-title-row-v179"><label class="schedule-title-first-v150">일정 이름<input name="title" maxlength="100" required placeholder="일정 이름을 입력하세요"></label><label class="schedule-all-day-v179"><input name="allDay" type="checkbox">종일</label></div><input type="hidden" name="recordScope" value="schedule"><div class="schedule-form-grid-v125"><label>시작 날짜<input name="date" type="date" required></label><label>종료 날짜<input name="endDate" type="date"></label><label>시작 시간<input name="time" type="time"></label><label>종료 시간<input name="endTime" type="time"></label><label>알림<select name="reminderMinutes"><option value="-1">알림 없음</option><option value="0">정각</option><option value="10">10분 전</option><option value="30" selected>30분 전</option><option value="60">1시간 전</option><option value="1440">하루 전</option></select></label><label class="schedule-check-v125" data-app-friend-slot-v179><input name="shareWithCouple" type="checkbox"><span>커플과 공유</span></label></div><label data-schedule-location-v150>장소<input name="location" maxlength="180" placeholder="장소 또는 링크"></label><label>메모<textarea name="note" maxlength="1200" placeholder="준비할 내용이나 상세 정보를 적어보세요."></textarea></label><p class="schedule-editor-error-v179" data-schedule-error-v179 role="status" aria-live="polite"></p><div class="schedule-dialog-actions-v125"><button type="button" class="danger" data-schedule-delete-v125 hidden>삭제</button></div></form></div></section>`;document.body.append(overlay);
