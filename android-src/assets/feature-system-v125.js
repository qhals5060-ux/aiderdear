(function () {
  ...
  function privateScheduleRowsV148() {
    return [...(window.AiderWorkCalendarV168?.rows?.(P)||[]),...(window.AiderEstateCalendarV171?.rows?.()||[])];
  }
  window.addEventListener('aiderlog-calendar-projection-v168',()=>{if(typeof activePage!=='undefined'&&activePage==='home')renderHome()});
  function scheduleRowsV125() { return [...baseScheduleRowsV125(),...privateScheduleRowsV148()].map(row=>row.projectionSource||['work','consulting','estate'].includes(row.calendarScope)?window.AiderBusinessCalendarV175.minimal({...row,projectionSource:row.projectionSource||row.calendarScope}):row); }
  ...
  function renderScheduleV125() {
    ...
    home.innerHTML=`<div class="home schedule-home-v119"><article class="calendar card schedule-calendar-v119"><div class="calhead schedule-calhead-v119"><h1><span>${MONTHS[month]}</span><small>${year}</small></h1><div class="calctl schedule-calctl-v119" aria-label="캘린더 조작"><button type="button" data-calendar-shift-v125="-1" aria-label="${range.mode==='fortnight'?'이전 2주':'이전 달'}">${icon('previous')}</button><button type="button" class="schedule-today-v119" data-calendar-today-v125>Today</button><button type="button" data-calendar-shift-v125="1" aria-label="${range.mode==='fortnight'?'다음 2주':'다음 달'}">${icon('next')}</button><button type="button" class="schedule-icon-v119 calendar-view-v176" data-calendar-view-v176 aria-label="${range.mode==='fortnight'?'월간 보기로 전환':'2주 보기로 전환'}" aria-pressed="${range.mode==='fortnight'}" title="${range.mode==='fortnight'?'2주 보기 · 월간으로 전환':'월간 보기 · 2주로 전환'}">W</button><button type="button" class="schedule-icon-v119 insights" data-schedule-insights-v125 aria-label="인사이트">${window.AiderLogIconsV126?.icon('insights')||icon('add')}</button></div></div><div class="week schedule-week-v119">${WEEK.map(day=>`<span>${day}</span>`).join('')}</div><div class="days schedule-days-v119">${scheduleCellsV125(year,month)}</div></article><aside class="homeside schedule-homeside-v119">${scheduleUpcomingV125()}${window.AiderAppDdayV175?.cardMarkup?.()||'<button type="button" class="dday schedule-dday-v119" data-dday-open-v125>D-DAY</button>'}</aside></div>`;
    bindScheduleV125();window.AiderLogV145?.decorateCalendar();window.AiderPrivateCalendarUIV175?.calendarChanged();window.AiderEstateCalendarV171?.refresh();
  }

  function ensureScheduleDialogV125() {
    let overlay=$('.schedule-dialog-v125'); if (overlay) return overlay;
    overlay=document.createElement('div');overlay.className='schedule-dialog-v125';overlay.innerHTML=`<section role="dialog" aria-modal="true"><header class="feature-dialog-head-v125"><div><small>SCHEDULE</small><h2 data-schedule-title-v125>일정 추가</h2></div><button type="button" data-schedule-dialog-close-v125 aria-label="닫기">${icon('close')}</button></header><div class="schedule-dialog-body-v125"><div class="schedule-day-list-v125" data-schedule-day-list-v125></div><form class="schedule-form-v125" data-schedule-form-v125><input type="hidden" name="id"><label class="schedule-title-first-v150">일정 이름<input name="title" maxlength="100" required placeholder="일정 이름을 입력하세요"></label><div class="schedule-form-grid-v125 schedule-source-grid-v148"><label>저장 위치<select name="recordScope"><option value="schedule">개인 일정</option></select></label><label data-schedule-client-v148 hidden>Consulting 고객<select name="clientId"></select></label></div><div class="schedule-work-fields-v150" data-schedule-work-v150 hidden><label>업무 상태<select name="workStatus"><option>예정</option><option>진행 중</option><option>검토 필요</option><option>대표 확인 필요</option><option>완료</option><option>보류</option></select></label><label>우선순위<select name="workPriority"><option>보통</option><option>높음</option><option>긴급</option><option>낮음</option></select></label><label>담당자<input name="workAssignee" maxlength="60" value="대표"></label><label>다음 행동<input name="workNextAction" maxlength="180" placeholder="업무를 이어갈 다음 행동"></label></div><div class="schedule-form-grid-v125"><label>시작 날짜<input name="date" type="date" required></label><label>종료 날짜<input name="endDate" type="date"></label><label>시작 시간<input name="time" type="time"></label><label>종료 시간<input name="endTime" type="time"></label><label>알림<select name="reminderMinutes"><option value="-1">알림 없음</option><option value="0">정각</option><option value="10">10분 전</option><option value="30" selected>30분 전</option><option value="60">1시간 전</option><option value="1440">하루 전</option></select></label><label class="schedule-check-v125"><input name="shareWithCouple" type="checkbox"><span>연결 상대와 공유</span></label></div><label data-schedule-location-v150>장소<input name="location" maxlength="180" placeholder="장소 또는 링크"></label><label>메모<textarea name="note" maxlength="1200" placeholder="준비할 내용이나 상세 정보를 적어보세요."></textarea></label><div class="schedule-dialog-actions-v125"><button type="button" class="danger" data-schedule-delete-v125 hidden>삭제</button><button type="button" data-schedule-dialog-close-v125>취소</button><button type="submit" class="primary">저장</button></div></form></div></section>`;document.body.append(overlay);
  ...
  function openScheduleV125(date,id='') {
    ...
    scheduleSelectedV125=date||row?.date||scheduleSelectedV125;form.reset();form.elements.id.value=row?.id||'';form.elements.recordScope.value=row?.calendarScope||'schedule';form.elements.clientId.innerHTML=(Array.isArray(P?.consultingClients)?P.consultingClients:[]).map(client=>`<option value="${safe(client.id)}">${safe(client.name||'이름 없음')}</option>`).join('');form.elements.clientId.value=row?.clientId||form.elements.clientId.options[0]?.value||'';form.elements.date.value=row?.date||scheduleSelectedV125;form.elements.endDate.value=row?.endDate||'';form.elements.time.value=row?.time||'';form.elements.endTime.value=row?.endTime||'';form.elements.reminderMinutes.value=String(row?.reminderMinutes??30);form.elements.title.value=row?.title||'';form.elements.location.value=row?.location||'';form.elements.note.value=row?.note||row?.memo||'';form.elements.shareWithCouple.checked=!!(row?.shareWithCouple||row?.owner==='shared');form.elements.shareWithCouple.disabled=!currentState().pair;const workRow=row?.calendarScope==='work'?(P.workRecords||[]).find(item=>String(item.id)===String(row.sourceId)):null;form.elements.workStatus.value=workRow?.status||'예정';form.elements.workPriority.value=workRow?.priority||'보통';form.elements.workAssignee.value=workRow?.assignee||'대표';form.elements.workNextAction.value=workRow?.nextAction||'';form.elements.recordScope.disabled=Boolean(row?.calendarScope);$('[data-schedule-title-v125]',overlay).textContent=row?`${row.calendarScope==='consulting'?'Consulting':row.calendarScope==='work'?'Work':'일정'} 수정`:`${scheduleSelectedV125} 일정 추가`;$('[data-schedule-delete-v125]',overlay).hidden=!row||row.readOnly;syncScheduleScopeV148();
    $('[data-schedule-day-list-v125]',overlay).innerHTML=rows.map(item=>`<article class="schedule-item-v125${receivedScheduleV176(item)?' schedule-received-v176':''}"${receivedScheduleV176(item)?` aria-label="상대가 공유한 일정 · ${safe(item.title||'일정')}"`:''}><i style="--event-color:${safe(eventColorV125(item))}"></i><div><b>${safe(item.title||'일정')}</b><span>${safe(item.allDay?'종일':item.time||'시간 미정')}${item.sourceTitle?` · ${safe(item.sourceTitle)}`:''}</span></div><button type="button" data-schedule-list-edit-v125="${safe(item.id)}">${item.readOnly?'보기':'수정'}</button></article>`).join('');
    $$('input,textarea,select,button[type="submit"]',form).forEach(control=>control.disabled=!!row?.readOnly && control.tagName.toLowerCase()!=='button');
  ...
  async function persistScheduleV125() {
    ...
    if (typeof saveApp === 'function') await saveApp();
    const api=window.AiderDearFirebase;if(currentUser()&&api?.writeScheduleData)try{await api.writeScheduleData(baseScheduleRowsV125())}catch(error){console.warn('Schedule sync skipped',error)}
  }
  ...
  async function saveScheduleV125(event) {
    event.preventDefault();const form=event.currentTarget,data=new FormData(form),id=String(data.get('id')||''),existing=id?scheduleRowsV125().find(row=>String(row.id)===id):null;if(existing?.readOnly)return;const start=String(data.get('date')||scheduleSelectedV125),end=String(data.get('endDate')||'');if(end&&end<start){alert('종료 날짜는 시작 날짜 이후여야 합니다.');return}const stamp=Date.now(),user=currentUser()||{},shared=!!data.get('shareWithCouple'),scope=existing?.calendarScope||String(data.get('recordScope')||'schedule'),title=String(data.get('title')||'').trim().slice(0,100),time=String(data.get('time')||''),endTime=String(data.get('endTime')||''),allDay=!time&&!endTime,note=String(data.get('note')||'').trim().slice(0,1200);if(!title)return;
    if(scope!=='schedule'){alert('업무 일정은 원래 업무 페이지에서 등록하거나 수정해주세요.');return;}
    const row={...existing,id:existing?.id||`schedule-${stamp}-${Math.random().toString(36).slice(2,7)}`,date:start,endDate:end,time,endTime,allDay,reminderMinutes:Number(data.get('reminderMinutes')??-1),title,category:existing?.category||'other',location:String(data.get('location')||'').trim().slice(0,180),note,owner:shared?'shared':'mine',shareWithCouple:shared,authorEmail:existing?.authorEmail||user.email||'',authorUid:existing?.authorUid||user.uid||'',createdAt:existing?.createdAt||stamp,updatedAt:stamp};const rows=baseScheduleRowsV125(),index=rows.findIndex(item=>String(item.id)===String(row.id));if(index>=0)rows[index]=row;else rows.push(row);scheduleSelectedV125=row.date;scheduleCursorV125=scheduleViewV176.focus(row.date).anchor;closeScheduleV125();renderScheduleV125();await persistScheduleV125();if(row.reminderMinutes>=0)syncNativeReminderV136(row);else cancelNativeReminderV136(row.id);
  }
  async function deleteScheduleV125(){const form=$('[data-schedule-form-v125]'),id=form?.elements.id.value,row=scheduleRowsV125().find(item=>String(item.id)===String(id));if(!row||row.readOnly||row.calendarScope||!confirm('이 기록을 삭제할까요?'))return;if(row.calendarScope==='consulting'){P.consultingTasks=(P.consultingTasks||[]).filter(item=>String(item.id)!==String(row.sourceId));await savePrivate?.()}else if(row.calendarScope==='work'){P.workRecords=(P.workRecords||[]).filter(item=>String(item.id)!==String(row.sourceId));await savePrivate?.()}else{A.scheduleEvents=baseScheduleRowsV125().filter(item=>String(item.id)!==String(id));cancelNativeReminderV136(id);await persistScheduleV125()}closeScheduleV125();renderScheduleV125();}

  function openDdayV125(){return window.AiderAppDdayV175?.open();}
  function bindScheduleV125(){home.querySelectorAll('[data-calendar-shift-v125]').forEach(button=>button.onclick=()=>{scheduleCursorV125=scheduleViewV176.move(button.dataset.calendarShiftV125).anchor;renderScheduleV125()});$('[data-calendar-today-v125]',home).onclick=()=>{const now=new Date();scheduleCursorV125=scheduleViewV176.today().anchor;scheduleSelectedV125=dateKey(now);renderScheduleV125()};$('[data-schedule-insights-v125]',home).onclick=()=>go('insights');$('[data-calendar-view-v176]',home).onclick=()=>{scheduleViewV176.toggle(scheduleSelectedV125);renderScheduleV125()};home.querySelectorAll('[data-schedule-edit-v125]').forEach(button=>button.onclick=()=>{const row=scheduleRowsV125().find(item=>String(item.id)===String(button.dataset.scheduleEditV125));openScheduleV125(row?.date||scheduleSelectedV125,row?.id||'')});home.querySelectorAll('[data-dday-open-v125]').forEach(button=>button.onclick=openDdayV125)}
