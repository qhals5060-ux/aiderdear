(function () {
  ...
  function ensureTravelFolderDialogV125() {
    ...
    $('[data-travel-folder-form-v125]',overlay).addEventListener('submit',async event=>{event.preventDefault();const data=new FormData(event.currentTarget),name=String(data.get('name')||'').trim(),start=String(data.get('startDate')||''),end=String(data.get('endDate')||''),error=$('[data-travel-folder-error-v125]',overlay);error.textContent='';if(!name)return;if(start&&end&&end<start){error.textContent='도착일은 출발일 이후여야 합니다.';return}const rows=travelFoldersV111().filter(row=>String(row.id)!=='trip-default'||A.travelFolders?.length);if(rows.some(row=>String(row.name||'').toLowerCase()===name.toLowerCase())){error.textContent='같은 이름의 여행지가 있습니다.';return}const colors=['#C7D8D0','#C9DAEB','#E8D4B5','#EFC9B5','#D9CFEC','#E9C9D1'],folder={id:`trip-${Date.now()}`,name:name,color:colors[rows.length%colors.length],startDate:start,endDate:end,createdAt:Date.now()};rows.push(folder);putTravelFoldersV111(rows);travelFolderV111=folder.id;overlay.classList.remove('on');event.currentTarget.reset();await saveApp();renderEvent()});return overlay;
  }
