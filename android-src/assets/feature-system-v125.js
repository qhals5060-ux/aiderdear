(function () {
  ...
  function privateScheduleRowsV148() {
    return window.AiderWorkCalendarV168?.rows?.(P)||[];
  }
  ...
  function openScheduleV125(date,id='') {
    ...
    if(projected){closeScheduleV125();window.AiderAppConsultWorkV168?.openCalendarRecord(projected.projectionSource,projected.sourceId)?.catch(error=>alert(error.message||'연결 기록을 열지 못했습니다.'));return}
  ...
  async function saveScheduleV125(event) {
    ...
    if(scope==='consulting'||scope==='work'){
      const app=window.AiderAppConsultWorkV168,seed={title,date:start,endDate:end,time,endTime,note,clientId:String(data.get('clientId')||'')};
      if(!app)return alert('업무 화면이 준비되지 않았습니다. 잠시 후 다시 시도해주세요.');
      if(scope==='consulting'&&!seed.clientId)return alert('Consult 고객을 먼저 선택해주세요.');
      try{scope==='work'?await app.newWorkTask(seed):await app.newConsultTask(seed);closeScheduleV125()}catch(error){alert(error.message||'기록 화면을 열지 못했습니다.')}return;
    }
