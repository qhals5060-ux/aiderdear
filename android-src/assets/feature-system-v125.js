(function () {
  ...
  function privateScheduleRowsV148() {
    const clients=new Map((Array.isArray(P?.consultingClients)?P.consultingClients:[]).map(row=>[String(row.id),row]));
    const consulting=(Array.isArray(P?.consultingTasks)?P.consultingTasks:[]).filter(row=>row?.dueDate).map(row=>({
      id:`consulting:${row.id}`,sourceId:String(row.id),calendarScope:'consulting',title:row.title||'Consulting 과업',date:String(row.dueDate).slice(0,10),endDate:'',time:row.dueTime||row.time||'',endTime:'',allDay:!(row.dueTime||row.time),category:'consulting',note:row.note||'',location:'',sourceTitle:`Consulting${clients.get(String(row.clientId))?.name?` · ${clients.get(String(row.clientId)).name}`:''}`,sourceColor:'#B56A46',clientId:row.clientId||'',readOnly:false
    }));
    const work=(Array.isArray(P?.workRecords)?P.workRecords:[]).filter(row=>row?.dueDate||row?.date).map(row=>({
      id:`work:${row.id}`,sourceId:String(row.id),calendarScope:'work',title:row.title||'Work 업무',date:String(row.dueDate||row.date).slice(0,10),endDate:String(row.endDate||''),time:row.dueTime||row.time||'',endTime:row.endTime||'',allDay:!(row.dueTime||row.time),category:'work',note:row.note||row.nextAction||'',location:row.location||'',sourceTitle:'Work',sourceColor:'#6255E8',readOnly:false
    }));
    return [...consulting,...work];
  }
  function scheduleRowsV125() { return [...baseScheduleRowsV125(),...privateScheduleRowsV148()]; }
  ...
  async function saveScheduleV125(event) {
    ...
    if(scope==='consulting'){P.consultingTasks=Array.isArray(P.consultingTasks)?P.consultingTasks:[];const target=P.consultingTasks.find(item=>String(item.id)===String(existing?.sourceId));if(target)Object.assign(target,{title,dueDate:start,dueTime:time,note,updatedAt:stamp});else{const clientId=String(data.get('clientId')||'');if(!clientId)return alert('Consulting 고객을 먼저 선택해주세요.');P.consultingTasks.push({id:`consult-task-${stamp}`,clientId,title,dueDate:start,dueTime:time,note,done:false,createdAt:stamp,updatedAt:stamp})}await savePrivate?.();closeScheduleV125();renderScheduleV125();return}
    if(scope==='work'){P.workRecords=Array.isArray(P.workRecords)?P.workRecords:[];const workPatch={title,dueDate:start,dueTime:time,note,status:String(data.get('workStatus')||'예정'),priority:String(data.get('workPriority')||'보통'),assignee:String(data.get('workAssignee')||'대표').trim().slice(0,60)||'대표',nextAction:String(data.get('workNextAction')||'').trim().slice(0,180),updatedAt:stamp};const target=P.workRecords.find(item=>String(item.id)===String(existing?.sourceId));if(target)Object.assign(target,workPatch);else P.workRecords.push({id:`work-${stamp}`,type:'업무',area:'운영',...workPatch,createdAt:stamp});await savePrivate?.();closeScheduleV125();renderScheduleV125();return}
