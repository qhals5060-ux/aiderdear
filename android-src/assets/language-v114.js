...
`;
  ...
  function enhanceLanguageLab(lab) {
    ...
    if (!lab.hasAttribute('data-training-v166') && !root.getElementById(SHADOW_STYLE_ID)) {
      const style = document.createElement('style');
    ...
    const syncQuick = () => {
      ...
      const meta=`${day} · ${lesson}`,label=action?.textContent?.trim() === '복습' ? '복습하기' : '시작하기';
      if(metaEl.textContent!==meta)metaEl.textContent=meta;
      if(button.textContent!==label)button.textContent=label;
      if(button.disabled!==!action)button.disabled = !action;
    };
