(function () {
  ...
  function ensureData() {
    ...
    P.labNotebookLinks = Array.isArray(P.labNotebookLinks) ? P.labNotebookLinks : [];
    P.labNotebookEntries = Array.isArray(P.labNotebookEntries) ? P.labNotebookEntries : [];
  ...
  function installPaperBridge() {
    window.AiderPaperBridge = {
      ...
      async saveCapture(entry) {
        const data=ensureData(),type=String(entry?.type||'note'),content=String(entry?.content||'').trim();
        if(!content)throw new Error('보관할 내용을 입력해주세요.');
        const row={id:String(entry?.id||uid('capture')),paperId:String(entry?.paperId||''),title:type==='question'?'연구 질문':type==='idea'?'아이디어 후보':type==='doi'?'DOI 보관':type==='url'?'논문 링크':'모바일 메모',content,type,status:type==='idea'?'ideaCandidate':'inbox',source:'mobile-capture',createdAt:Number(entry?.createdAt)||Date.now(),updatedAt:Date.now()};
        const at=data.researchNotes.findIndex(item=>item.id===row.id);if(at>=0)data.researchNotes.splice(at,1,row);else data.researchNotes.unshift(row);
        await persist('사이트 PAPER로 동기화했습니다.');
