...
export function createEstateClient(onIdentityChange=()=>{}) {
  ...
  const scope=()=>{const uid=identity();if(!uid)throw Error('ESTATE는 지정된 두 계정에서만 사용할 수 있습니다.');return {uid,epoch:generation};};
  const check=owner=>{if(identity()!==owner.uid||generation!==owner.epoch)throw Object.assign(Error('로그인 계정이 변경되었습니다. 다시 열어주세요.'),{code:'estate-identity-changed'});};
  // Keep retries, every file chunk and thumbnail cleanup in the starting session.
  // A logout/relogin event also invalidates work when the UID returns to its old value.
  ['aiderdear-firebase-ready','aiderdear-firebase-state'].forEach(name=>window.addEventListener(name,identity));
  async function call(action,payload={},owner=scope()) {
    check(owner);
    ...
    for(let attempt=0;attempt<2;attempt++) {
      try {
        ...
        return data;
      } catch(error) {check(owner);if(attempt||error.status&&error.status<500)throw error;}
    }
  }
  ...
  async function sendFile(file,target,owner) {
    check(owner);
    if(file.size>12*1024*1024)throw Error('파일 하나는 12MB 이하로 첨부해주세요.');
    const start=await call('mediaBegin',{...target,name:file.name,type:file.type||'application/octet-stream',size:file.size},owner);
    const chunkBytes=start.chunkBytes||384*1024;
    try{for(let offset=0,index=0;offset<file.size;offset+=chunkBytes,index++){check(owner);const bytes=new Uint8Array(await file.slice(offset,offset+chunkBytes).arrayBuffer());let s='';for(let i=0;i<bytes.length;i+=8192)s+=String.fromCharCode(...bytes.subarray(i,i+8192));await call('mediaChunk',{id:start.id,index,data:btoa(s)},owner);}return await call('mediaFinish',{id:start.id},owner);}
    catch(error){try{await call('mediaDelete',{id:start.id},owner);}catch{}throw error;}
  }
  async function upload(file,target) {
    const owner=scope(),compressed=await compress(file),saved=await sendFile(compressed,target,owner);
    if(target.entityCollection==='properties'&&target.thumbnail!==false&&/^image\/(jpeg|png|webp)$/.test(compressed.type))try{const thumbnail=await sendFile(await compress(compressed,360,.72),target,owner);saved.thumbId=thumbnail.id;}catch(error){try{await call('mediaDelete',{id:saved.id},owner);}catch{}throw error;}
    check(owner);
    return {...saved,name:compressed.name,type:compressed.type,size:compressed.size};
  }
  async function download(id,owner=scope()) {
    const info=await call('mediaInfo',{id},owner),parts=[];
    for(let index=0;index<Math.ceil(info.size/(info.chunkBytes||384*1024));index++){const part=await call('mediaReadChunk',{id,index},owner);parts.push(Uint8Array.from(atob(part.data),c=>c.charCodeAt(0)));}
  ...
  async function image(id){const owner=scope();if(urls.has(id))return urls.get(id);const {blob}=await download(id,owner);check(owner);if(urls.has(id))return urls.get(id);if(!/^image\/(jpeg|png|webp)$/.test(blob.type))throw Error('지원하지 않는 이미지입니다.');const url=URL.createObjectURL(blob);urls.set(id,url);return url;}
