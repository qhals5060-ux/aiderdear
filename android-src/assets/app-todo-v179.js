...
(() => {
  ...
  const subtasks=row=>Array.isArray(row?.subtasks)?row.subtasks:[];
  const subtaskMarkup=row=>`<div class="todo-subtask-row-v186" data-todo-subtask-v186="${safe(row.id)}"><input type="hidden" name="subtask-id" value="${safe(row.id)}"><input type="checkbox" name="subtask-done-${safe(row.id)}" ${row.done?'checked':''} aria-label="하위 할 일 완료"><input name="subtask-text-${safe(row.id)}" type="text" value="${safe(row.text)}" maxlength="180" required aria-label="하위 할 일 내용" placeholder="하위 할 일"><button type="button" data-todo-subtask-remove-v186 aria-label="하위 할 일 삭제">×</button></div>`;
  ...
  function render(){
    ...
    const markup=rows.map(row=>`<article class="todo-row-v179 ${row.done?'is-done':''}" data-todo-row-kind-v186="${kind}">${kind==='todo'?`<label class="todo-check-v179"><input type="checkbox" data-todo-check-v179="${safe(row.id)}" data-source="${row.source}" ${row.done?'checked':''} ${busy?'disabled':''} aria-label="${safe(row.text)} 완료"></label>`:''}<button type="button" class="todo-copy-v179" data-todo-edit-v179="${safe(row.id)}" data-source="${row.source}">${kind==='todo'?`<b title="${safe(row.text)}">${row.important?'<span class="todo-important-mark-v186" aria-label="중요">·</span>':''}${safe(row.text)}</b>${subtasks(row).length?`<small class="todo-progress-v186" aria-label="하위 할 일 ${subtasks(row).filter(item=>item.done).length}개 완료, 전체 ${subtasks(row).length}개">${subtasks(row).filter(item=>item.done).length}/${subtasks(row).length}</small>`:''}${row.date?`<time class="todo-due-v186 ${row.date<day()&&!row.done?'is-overdue':''}" datetime="${safe(row.date)}" title="${safe(row.date)} 마감">${due(row.date)} 마감</time>`:''}`:`<b>${safe(row.text)}</b>${row.notes||row.note?`<span>${safe(row.notes||row.note)}</span>`:''}${row.updatedAt?`<small>${new Date(row.updatedAt).toLocaleDateString('ko-KR')}</small>`:''}`}</button><button class="todo-more-v179" type="button" data-todo-edit-v179="${safe(row.id)}" data-source="${row.source}" aria-label="${safe(row.text)} 수정">⋯</button></article>`).join('')||(!loading&&active?`<div class="todo-empty-v179">${query?'검색 결과가 없습니다.':kind==='memo'?'저장한 메모가 없습니다.':filter==='done'?'완료한 할 일이 없습니다.':'표시할 할 일이 없습니다.'}</div>`:'');
  ...
  function edit(source='',id='',requestedKind=''){
    ...
    sheet.innerHTML=`<section class="todo-editor-v179" role="dialog" aria-modal="true" aria-labelledby="todo-editor-title-v179"><form data-todo-form-v179><header><h2 id="todo-editor-title-v179">${editingKind==='memo'?'메모':'투두'}</h2><button type="submit">저장</button><button type="button" data-todo-close-v179 aria-label="닫기">×</button></header><div class="todo-editor-fields-v179"><label>${editingKind==='memo'?'메모':'할 일'}<textarea name="text" maxlength="${editingKind==='memo'?1200:180}" rows="${editingKind==='memo'?5:1}" required>${safe(todoTextV179(before))}</textarea></label>${editingKind==='todo'?`<div class="todo-editor-options-v179"><label>마감일<input name="date" type="date" value="${safe(before?.date||before?.dueAt||'')}"></label><label class="todo-important-v179"><input type="checkbox" name="important" ${before?.important?'checked':''}> 중요</label></div><section class="todo-subtasks-v186" aria-label="하위 할 일"><div data-todo-subtask-list-v186>${subtasks(before).map(subtaskMarkup).join('')}</div><button type="button" data-todo-subtask-add-v186>＋ 하위 할 일</button></section>`:''}<label>상세 메모<textarea name="notes" maxlength="2000" rows="${editingKind==='memo'?3:2}">${safe(before?.notes||before?.note||'')}</textarea></label><p class="todo-form-error-v179" role="alert"></p>${before?'<button type="button" class="todo-delete-v179" data-todo-delete-v179>삭제</button>':''}</div></form></section>`;
    ...
    sheet.addEventListener('click',event=>{
      if(event.target===sheet||event.target.closest('[data-todo-close-v179]'))close();
      if(busy)return;
      if(event.target.closest('[data-todo-subtask-add-v186]')){
        const list=$('[data-todo-subtask-list-v186]',sheet);if(!list)return;
        if(list.children.length>=20){$('.todo-form-error-v179',sheet).textContent='하위 할 일은 20개까지 추가할 수 있습니다.';return;}
        list.insertAdjacentHTML('beforeend',subtaskMarkup({id:freshId('subtask'),text:'',done:false}));
        list.lastElementChild?.querySelector('input[type="text"]')?.focus({preventScroll:true});
        list.lastElementChild?.scrollIntoView({block:'nearest',behavior:'auto'});
      }
      const remove=event.target.closest('[data-todo-subtask-remove-v186]');if(remove)remove.closest('[data-todo-subtask-v186]')?.remove();
    });
