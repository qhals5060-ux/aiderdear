...
(() => {
  ...
  let owner='',ownerEpoch=0,data={checklists:[],memos:[]},kind='todo',filter='open',query='',loaded=false,loading=false,busy=false,error='',loadRun=0,writeVersion=0,boundApi=null,queued=false;
  ...
  function ensurePage(){
    ...
    if(!page.querySelector('[data-todo-workspace-v179]'))page.innerHTML=`<div class="todo-workspace-v179" data-todo-workspace-v179><header class="todo-header-v179"><div><span>NOTEBOOK</span><h1>투두 · 메모</h1></div><button type="button" data-todo-add-v179>＋ 추가</button></header><nav class="todo-tabs-v179" aria-label="노트 종류"><button type="button" data-todo-kind-v179="todo">투두</button><button type="button" data-todo-kind-v179="memo">메모</button></nav><div class="todo-toolbar-v179"><label class="todo-search-v179"><span class="todo-sr-v179">투두와 메모 검색</span><input type="search" data-todo-search-v179 placeholder="내용 검색" autocomplete="off"></label><select data-todo-filter-v179 aria-label="할 일 상태"><option value="open">미완료</option><option value="today">오늘 마감</option><option value="overdue">기한 지남</option><option value="all">전체</option><option value="done">완료</option></select><button type="button" data-todo-refresh-v179 aria-label="새로고침">↻</button></div><div class="todo-status-v179" data-todo-status-v179 role="status"></div><div class="todo-list-v179" data-todo-list-v179></div></div>`;
    return page;
  }
  function render(){
    ...
    $('[data-todo-add-v179]',page).textContent=kind==='memo'?'＋ 메모':'＋ 투두';
    $('[data-todo-add-v179]',page).disabled=!active||!loaded||busy;
  ...
  function mountInline(element){if(!element)return;const markup=inlineMarkup();element.hidden=!incomplete().length&&!error;if(signatures.get(element)===markup)return;const scroll=element.firstElementChild?.scrollTop||0;element.innerHTML=markup;if(element.firstElementChild)element.firstElementChild.scrollTop=scroll;signatures.set(element,markup);}
  ...
  function changeOwner(next){if(owner===next)return;owner=next;++ownerEpoch;++loadRun;++writeVersion;data={checklists:[],memos:[]};loaded=false;loading=false;busy=false;error='';query='';document.querySelector('[data-todo-editor-v179]')?.remove();render();const search=$('[data-todo-search-v179]');if(search)search.value='';}
  ...
  function edit(source='',id=''){
    ...
    const editingKind=before?todoKindV179(before,source):kind,collection=source||(editingKind==='memo'?'memos':'checklists'),recordId=id||freshId(editingKind);
    ...
    const close=()=>{if(busy)return;sheet.remove();previousFocus?.focus?.({preventScroll:true});};
  ...
  document.addEventListener('click',event=>{
    ...
    else if(button.hasAttribute('data-todo-add-v179'))edit();
  ...
  window.AiderTodoV179=Object.freeze({open,refresh,incomplete,inlineMarkup,mountInline,mountAll,edit});
