(function () {
  ...
  const emotionMultiSelectV152 = (name,values) => `<details class="emotion-multi-select-v152" data-emotion-multi-v152>
    <summary><span data-emotion-multi-label-v152>선택</span><i aria-hidden="true"></i></summary>
    <div class="emotion-multi-options-v152">${values.map(value=>`<label><input type="checkbox" name="${name}" value="${value}"><span>${value}</span></label>`).join('')}</div>
  </details>`;
  ...
  function ensureEmotionDialog() {
    ...
    overlay.innerHTML = `<section role="dialog" aria-modal="true" aria-labelledby="emotionDialogTitleV119">
      ...
      <form data-emotion-form-v119>
        ...
        <fieldset><legend>지금 가장 가까운 감정 <small>(복수 선택)</small></legend><div class="emotion-options-v119 emotion-characters-v126">${MOODS.map(([mood],index) => `<label><input type="checkbox" name="moods" value="${mood}" ${index===3?'checked':''}><span><img src="./mascots-v118/${EMOTION_FILES_V126[mood]}.png" alt=""><b>${mood}</b></span></label>`).join('')}</div></fieldset>
        ...
        <fieldset class="emotion-block-v126"><legend>무슨 일이 있었나요? <small>(복수 선택)</small></legend>${emotionMultiSelectV152('context',EMOTION_CONTEXTS_V126)}</fieldset>
        <fieldset class="emotion-block-v126"><legend>그때 무엇을 하고 있었나요? <small>(복수 선택)</small></legend>${emotionMultiSelectV152('currentActivities',EMOTION_CURRENT_V126)}</fieldset>
        <fieldset class="emotion-block-v126"><legend>그 감정이 든 뒤 어떻게 했나요? <small>(복수 선택)</small></legend>${emotionMultiSelectV152('afterActivities',EMOTION_AFTER_V126)}</fieldset>
        ...
        <fieldset class="emotion-block-v126"><legend>지금 하고 싶은 것 <small>(복수 선택)</small></legend>${emotionMultiSelectV152('wants',EMOTION_WANTS_V126)}<input name="wantCustom" maxlength="120" placeholder="직접 입력 · 쉼표로 여러 개 입력할 수 있어요."></fieldset>
    ...
    document.body.append(overlay);
    overlay.addEventListener('click',event => { if (event.target === overlay || event.target.closest('[data-emotion-close-v119]')) closeEmotionDialogV176(); });
    const updateMultiSelect = details => {
      const selected = Array.from(details.querySelectorAll('input:checked')).map(input=>input.value);
      const label = details.querySelector('[data-emotion-multi-label-v152]');
      if (label) label.textContent = selected.length ? `${selected[0]}${selected.length>1?` 외 ${selected.length-1}`:''}` : '선택';
    };
    overlay.addEventListener('change',event=>{
      const details=event.target.closest('[data-emotion-multi-v152]');
      if(details)updateMultiSelect(details);
    });
    overlay.addEventListener('toggle',event=>{
      const details=event.target.closest?.('[data-emotion-multi-v152]');
      if(!details?.open)return;
      overlay.querySelectorAll('[data-emotion-multi-v152][open]').forEach(other=>{if(other!==details)other.open=false;});
    },true);
    overlay.querySelector('form').addEventListener('submit',saveEmotionV119);
    return overlay;
  }
  function closeEmotionDialogV176(){
    document.querySelector('[data-emotion-dialog-v119]')?.classList.remove('on');
    const onClose=emotionReturnV176;emotionReturnV176=null;if(onClose)onClose();
  }
  function openEmotionDialog(presetDate='',options={}) {
    const overlay = ensureEmotionDialog(), form = overlay.querySelector('form');
    emotionReturnV176=typeof options.onClose==='function'?options.onClose:null;
    form.reset(); form.elements.date.value = typeof presetDate==='string'&&/^\d{4}-\d{2}-\d{2}$/.test(presetDate)?presetDate:selectedDate || localDate(new Date());
    form.querySelector('input[name="moods"][value="편안함"]').checked = true;
    form.querySelector('input[name="intensity"][value="3"]').checked = true;
    form.querySelectorAll('[data-emotion-multi-v152]').forEach(details=>{details.open=false;const label=details.querySelector('[data-emotion-multi-label-v152]');if(label)label.textContent='선택';});
    overlay.classList.add('on');
  }
  async function saveEmotionV119(event) {
    ...
    if(!moods.length&&!lines.some(Boolean)&&!wants.length&&!context.length&&!currentActivities.length&&!afterActivities.length&&!location&&!companion){alert('기록할 내용을 선택하거나 입력해주세요.');return;}
    E.entries = Array.isArray(E?.entries) ? E.entries : Object.values(E?.entries || {});
    E.entries.push({id:`emotion-${stamp}`,date:String(data.get('date') || localDate(new Date())),time:String(data.get('time')||''),mood,moods,intensity:Number(data.get('intensity') || 3),location,place:location,companion,wants,context,currentActivities,afterActivities,nextEmotion:String(data.get('nextEmotion')||''),activity:currentActivities[0]||'',lines,note:lines.filter(Boolean).join(' / '),createdAt:stamp,updatedAt:stamp});
    try { localStorage.setItem('aiderlog-emotion-v119',JSON.stringify(E)); } catch (_) {}
    closeEmotionDialogV176();
  ...
  function bindScheduleSync() {
    ...
    api.subscribe(async state => {
      ...
      try {
        ...
        try { localStorage.setItem('aiderlog-app-v20',JSON.stringify(A)); } catch (_) {}
        // A late cloud response must use the installed calendar renderer,
        // not this legacy closure after v125 has taken ownership of Home.
  ...
  restoreEmotionLocal();
  window.AiderAppEmotionV176=Object.freeze({open:openEmotionDialog});
