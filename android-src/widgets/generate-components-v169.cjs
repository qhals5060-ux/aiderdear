...
for(const name of fs.readdirSync(path.join(res,'layout')).filter(n=>/^widget_.*\.xml$/.test(n))){
  ...
  if(!/widget_(?:design|native|item|note|todo|routine|stats|challenge|workout|book|quote|workflow|day|meal_slot|client_link|stack|group)/.test(name))continue;
  xml=xml.replace(/<(FrameLayout|LinearLayout)\b[^>]*>/,tag=>attrs(tag,{forceDarkAllowed:'false'}));
  if(/^widget_(note|todo|routine|stats|challenge|workout|book|quote|workflow)_v165/.test(name)){
  ...
  if(/^widget_(note|todo|routine|challenge|workout|workflow)_v165(?:_cell)?\.xml$/.test(name))xml=xml.replace(/<\/FrameLayout>\s*$/,separator()+'</FrameLayout>');
