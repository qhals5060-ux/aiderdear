...
async function layout(el,availableW,availableH,forcedW,forcedH){
  ...
  if(el.name==='FrameLayout'){
    ...
    for(const n of nodes){const margin=edge(n,'layout_margin');if(px(get(n,'layout_weight'))&&remaining!==undefined){totalWeight+=px(get(n,'layout_weight'));remaining-=margin.t+margin.b;}else{const child=await layout(n,cw,get(n,'layout_height')==='match_parent'&&remaining!==undefined?Math.max(0,remaining):undefined);pre.set(n,child);if(remaining!==undefined)remaining-=child.h+child.m.t+child.m.b;}}
...
function drawable(ref,x,y,w,h,scaleType){
  if(!ref||w<=0||h<=0)return '';
  if(ref.startsWith('#'))return `<rect x="${x}" y="${y}" width="${w}" height="${h}" fill="${color(ref)}"/>`;
  const key=ref.replace('@drawable/','');
  if(bundle.images[key])return `<image x="${x}" y="${y}" width="${w}" height="${h}" href="${bundle.images[key]}" preserveAspectRatio="${scaleType==='centerCrop'?'xMidYMid slice':scaleType==='fitXY'?'none':'xMidYMid meet'}"/>`;
