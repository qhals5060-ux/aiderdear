    ...
    @Override public void onReceive(Context c,Intent intent){
        ...
        if("month".equals(operation)){
            ...
            if("CalendarFortnight".equals(kind)){
                int offset=WidgetNativeV164.prefs(c).getInt("widget_fortnight_offset_"+id,0)+delta;
                WidgetNativeV164.prefs(c).edit().putInt("widget_fortnight_offset_"+id,offset).apply();
                WidgetNativeV164.update(c,AppWidgetManager.getInstance(c),id,kind);return;
            }
            if("CalendarAgenda".equals(kind)){
                int offset=WidgetNativeV164.prefs(c).getInt("widget_agenda_offset_"+id,0)+delta;
                WidgetNativeV164.prefs(c).edit().putInt("widget_agenda_offset_"+id,offset).apply();
                WidgetNativeV164.update(c,AppWidgetManager.getInstance(c),id,kind);return;
            }
            ...
            WidgetNativeV164.prefs(c).edit().putInt("widget_month_"+id,0).putInt("widget_fortnight_offset_"+id,0).putInt("widget_agenda_offset_"+id,0).putString("widget_date_"+id,WidgetNativeV164.day(Calendar.getInstance())).apply();
