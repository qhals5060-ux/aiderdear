...
function harness(initialUid='A', storage={}) {
  ...
  const doc={documentElement:{dataset:{theme:'saturn'}},body:{},hidden:false,addEventListener:(name,fn)=>{(events[name]??=[]).push(fn)},querySelector:()=>null,createElement:tag=>tag==='canvas'?{width:0,height:0,getContext:()=>({fillRect:()=>{},drawImage:(image)=>{doc.imageSource=image.src}}),toDataURL:()=>`data:image/jpeg;base64,${doc.imageSource}`}:{}};
...
(async()=>{
  ...
  const cover=harness('A');vm.runInContext(fs.readFileSync(path.join(__dirname,'widget-models-v165.js'),'utf8'),cover.ctx);
  cover.rows.A.personal.personalItems=[{id:'older',category:'reading',title:'Same book',date:'2026-09-01',createdAt:1,updatedAt:100,localImage:'data:image/jpeg;base64,OLD_COVER',details:{author:'Author'}},{id:'latest',category:'reading',title:'Same book',date:'2026-09-06',createdAt:2,updatedAt:10,localImage:'data:image/jpeg;base64,NEW_COVER',details:{author:'Author'}}];
  await cover.widget.refresh();assert.equal(cover.images.length,1);assert(cover.images[0].src.includes('NEW_COVER'));cover.images[0].onload();await tick();assert(cover.widget.snapshot().v165.books[0].image.includes('NEW_COVER'));
  console.log('PASS book thumbnail hydration uses the same date/create representative as the reading model');
