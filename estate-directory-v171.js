...
export function installDirectory(app){
  ...
  function stateFor(collection){
    ...
    if(!viewStates.has(collection))viewStates.set(collection,{rows:[],cursor:null,loaded:false,busy:false,q:'',mode:app.directoryMode==='cards'?'cards':'list',status:'',type:'',deal:'',role:'',region:'',coBroker:'',sort:'recent',...DETAIL_FILTERS,detailsOpen:false,global:null,sequence:0});
  ...
  async function propertyPanel({container,row,isNew,signal}){
    ...
    const owner=app.uid(),assertActive=()=>{if(signal?.aborted||app.uid()!==owner)throw Error('화면 또는 로그인 계정이 변경되어 첨부 작업을 중단했습니다.');};
    ...
    const form=app.form(container.querySelector('[data-directory-form]'),basic+physical+terms+special+management+brokerage+description+attachments,async(data,formElement)=>{
      ...
      assertActive();current=saved;refreshHistory(array(saved.history));const retained=new Set([...documents,...photos.flatMap(photo=>[photo.mediaId,photo.thumbId].filter(Boolean))]);
      for(const id of removed){assertActive();if(retained.has(id))continue;try{await app.api.call('mediaDelete',{id});}catch(error){assertActive();app.notice('첨부 연결은 제거했습니다. 다른 기록에서 사용 중인 원본 파일은 보존됩니다.');}}assertActive();removed.clear();if(isNew)offerSavedDetails(form,'properties',saved.id);return saved;
    });
    ...
    async function upload(files,photo){if(!files.length)return;const max=photo?20:30;if((photo?photos.length:documents.length)+files.length>max){app.notice(`${photo?'사진은':'서류는'} 최대 ${max}개까지 등록할 수 있습니다.`,true);return}pendingUploads++;form.querySelectorAll('button[type="submit"]').forEach(button=>button.disabled=true);form.querySelectorAll('input[type="file"]').forEach(input=>input.disabled=true);
      // Aborted panels leave unlinked files for the owner-scoped maintenance
      // cleanup; a new login must never perform the former panel's cleanup.
      try{const failed=[];for(let i=0;i<files.length;i++){assertActive();const file=files[i];try{if(photo&&!/^image\/(jpeg|png|webp)$/.test(file.type))throw Error('JPG, PNG, WebP 사진을 선택해주세요.');uploadStatus.textContent=`${i+1}/${files.length} 업로드 · ${file.name}`;const item=await app.api.upload(file,{entityCollection:'properties',entityId:current.id,thumbnail:photo});assertActive();if(photo)photos.push({mediaId:item.id,thumbId:item.thumbId||'',name:item.name||file.name});else{documents.push(item.id);mediaNames.set(item.id,item.name||file.name);}if(alive(form,signal)){dirtyMedia();renderPhotos();renderDocuments();}}catch(error){assertActive();failed.push(file.name+': '+(error.message||'업로드 실패'));}}if(alive(form,signal))uploadStatus.textContent=failed.length?`완료된 파일은 유지됩니다. 다음 파일만 다시 선택해주세요.\n${failed.join('\n')}`:'업로드 완료 · 저장하면 첨부가 연결됩니다.';}
      catch(error){if(alive(form,signal)&&app.uid()===owner){uploadStatus.textContent=error.message||'업로드하지 못했습니다.';app.notice(uploadStatus.textContent,true);}}
