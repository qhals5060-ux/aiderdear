window.initAiderPaperWorkspaceV128 = function initAiderPaperWorkspaceV128() {
  'use strict';

  const stage = document.querySelector('#paperStage');
  if (!stage) return;
  const mounted = stage.querySelector('aider-paper-workspace-v121');
  if (mounted) {
    window.AiderPaperWorkspace?.activate?.();
    return;
  }
  [...stage.children].forEach(child => {
    child.hidden = true;
    child.classList.add('paper-v121-legacy');
  });

  const paperHost = document.createElement('aider-paper-workspace-v121');
  const paperRoot = paperHost.attachShadow({ mode: 'open' });
  paperRoot.innerHTML = `
    <link rel="stylesheet" href="./paper-workspace-v128.css?v=145b">
    <div class="paper-v121-surface">
      <main class="paper-app">
        <aside class="paper-sidebar">
          <div class="paper-sidebar-tools">
            <label class="sidebar-search"><span>⌕</span><input id="globalSearch" type="search" placeholder="논문·저자·근거 검색"></label>
            <button type="button" class="sidebar-import" id="importButton">＋ 논문 가져오기</button>
            <button type="button" class="sidebar-guide" id="guideButton">사용 안내</button>
          </div>
          <nav class="paper-nav" id="paperNav" aria-label="Paper 연구 메뉴">
            <button type="button" class="active" data-view="hub"><span>01</span><b>오늘</b><small>연구 브리핑</small></button>
            <button type="button" data-view="library"><span>02</span><b>라이브러리</b><small>논문과 자료</small></button>
            <button type="button" data-view="evidence"><span>03</span><b>근거 검토</b><small>원문 검증</small><i id="evidenceBadge">3</i></button>
            <button type="button" data-view="synthesis"><span>04</span><b>문헌 분석</b><small>패턴과 공백</small></button>
            <button type="button" data-view="study" data-study-entry="idea"><span>05</span><b>아이디어</b><small>질문과 가설</small></button>
            <button type="button" data-view="study" data-study-entry="design"><span>06</span><b>연구 설계</b><small>방법과 실행</small></button>
          </nav>
      ...
      <aside class="import-drawer" id="importDrawer" aria-hidden="true" aria-labelledby="importTitle">
        ...
        <section class="import-section quick-collect-v145">
          <div class="section-heading compact"><div><span>QUICK COLLECT</span><h3>DOI · URL · PDF 또는 서지정보</h3></div></div>
          <form id="quickPaperForm" class="quick-paper-form-v145">
            <label class="wide">DOI 또는 URL<input name="doi" type="url" inputmode="url" placeholder="https://doi.org/… 또는 논문 URL"></label>
            <label class="wide">논문 제목<input name="title" placeholder="영문 원제 유지"></label>
            <label>저자<input name="authors" placeholder="Author et al."></label><label>연도<input name="year" inputmode="numeric" placeholder="2026"></label>
            <label class="wide">저널<input name="journal" placeholder="Journal name"></label>
            <label class="wide">PDF 등록<input name="pdf" type="file" accept="application/pdf,.pdf"><small>로그인 계정의 개인 저장공간에만 저장 · 최대 25MB</small></label>
            <label class="wide">메모<textarea name="note" rows="2" placeholder="핵심 메모 또는 확인할 질문"></textarea></label>
            <button type="submit" class="primary-button wide">Library에 수집</button>
          </form>
        </section>
  ...
  stage.classList.add('paper-v121-ready');
...
- MRI 좌표, atlas, threshold, smoothing, correction을 보이지 않는 경우 추정 금지`;
  ...
  const builtInPapers = papers.map(row => ({...structuredClone(row),isDemo:true,statusLabel:`DEMO · ${row.statusLabel}`}));
  ...
  function renderSynthesis() {
    ...
    return `<div class="view-stack">
      ${heading('SYNTHESIS', '문헌 통합 분석', '', '<button type="button" class="outline-button" data-go="atlas">뇌 지도 열기</button>')}
      <section class="research-card">
  ...
  function openImport() {
    if (performance.now() - mountedAt < 420) return;
  ...
  $('#paperNav').addEventListener('click', event => {
    ...
    if (button) {
      if (button.dataset.studyEntry === 'idea') studyStep = 0;
      if (button.dataset.studyEntry === 'design') studyStep = Math.max(1, studyStep);
  ...
  $('#quickPaperForm').addEventListener('submit', async event => {
    event.preventDefault();
    const form=event.currentTarget,button=form.querySelector('button[type="submit"]'),fields=Object.fromEntries(new FormData(form).entries()),file=form.elements.pdf.files?.[0]||null;
    delete fields.pdf;
    if (!String(fields.title||'').trim() && !String(fields.doi||'').trim() && !file) { showToast('논문 제목, DOI·URL 또는 PDF 중 하나를 입력해주세요.'); return; }
    if (!window.AiderPaperBridge?.saveQuickPaper) { showToast('AiderLog 로그인과 저장 연결을 확인해주세요.'); return; }
    button.disabled=true;button.textContent='수집 중…';
    try { const result=await window.AiderPaperBridge.saveQuickPaper(fields,file);syncBridgeCollections();form.reset();closeImport();openPaper(result.id);showToast('개인 Library에 논문을 수집했습니다.'); }
    catch(error){showToast(error.message||'논문을 수집하지 못했습니다.');}
    finally{button.disabled=false;button.textContent='Library에 수집';}
  });
